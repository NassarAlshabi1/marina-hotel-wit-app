...
const collections = [
  ...
  },
  {
    id: 'shift_notes',
    name: 'ملاحظات النوبة',
    description: 'جدول ملاحظات النوبات',
    attributes: [
      { key: 'note', type: 'string', size: 1000, required: true },
      { key: 'shiftDate', type: 'string', size: 50, required: true },
  ...
  },
  {
    id: 'outbox',
    name: 'الصندوق الصادر',
    description: 'جدول الرسائل الصادرة',
    attributes: [
      { key: 'message', type: 'string', size: 1000, required: true },
      { key: 'recipient', type: 'string', size: 200, required: true },
  ...
  },
  {
    id: 'sync_state',
    name: 'حالة المزامنة',
    description: 'جدول حالة المزامنة',
    attributes: [
      { key: 'isSyncing', type: 'boolean', required: true },
      { key: 'lastSyncTime', type: 'integer', required: true },
      { key: 'localUuid', type: 'string', size: 100, required: true, unique: true },
      { key: 'serverId', type: 'integer' },
      { key: 'createdAt', type: 'integer', required: true },
      { key: 'updatedAt', type: 'integer', required: true },
...
async function createAttribute(databaseId, collectionId, attribute) {
  try {
    switch (attribute.type) {
      ...
      case 'boolean':
        await databases.createBooleanAttribute(
          ...
          attribute.key,
          attribute.required || false,
          (attribute.required ? undefined : (attribute.default ?? false)),
          attribute.array || false
        );
        break;
      default:
    ...
    if (error?.code === 409) {
      ...
      console.error(`Failed to create attribute ${attribute.key} in ${collectionId}:`, error.response || error);
    }
  }
}
...
async function ensureCollection(collection) {
  try {
    ...
    if (error?.code === 409) {
      ...
      console.error(`✖ Failed to create collection ${collection.id}:`, error.response || error);
      return false;
    }
  }
...
(async () => {
  ...
  console.log('✅ Completed creating all collections and attributes.');
})();
