...
# ==========================================
# F4DD Automated Test Execution Script
...
clean_artifacts() {
    ...
    if [ -d "$REPORT_DIR" ]; then
        rm -rf "$REPORT_DIR"
        print_success "Removed report directory"
    fi
...
run_unit_tests() {
    ...
    if [ -d "$UNIT_TESTS" ]; then
        ...
        if flutter test "$UNIT_TESTS" --coverage --coverage-path="$COVERAGE_DIR/unit.lcov" 2>&1 | tee "test_results_unit.txt"; then
...
run_widget_tests() {
    ...
    if [ -d "$WIDGET_TESTS" ]; then
        ...
        if flutter test "$WIDGET_TESTS" --coverage --coverage-path="$COVERAGE_DIR/widget.lcov" 2>&1 | tee "test_results_widget.txt"; then
...
run_integration_tests() {
    ...
    if [ -d "$INTEGRATION_TESTS" ]; then
        ...
        if flutter test "$INTEGRATION_TESTS" --coverage --coverage-path="$COVERAGE_DIR/integration.lcov" 2>&1 | tee "test_results_integration.txt"; then
...
run_performance_tests() {
    ...
    if [ -d "$PERFORMANCE_TESTS" ]; then
        ...
        if flutter test "$PERFORMANCE_TESTS" --reporter=json > performance_report.json 2>&1; then
