...
8. ✅ **جدول outbox نفسه محلي فقط** — لا يُزامن مع Cloud أبداً

---

## 🚫 الجداول المحلية فقط (لا تُزامن)

الجداول التالية **محلية فقط** ولا يجب أن تُزامن مع Appwrite Cloud:

| # | الجدول | السبب |
|---|--------|-------|
| 1 | `outbox` | جدول الطابور — يُستخدم محلياً فقط لتتبع التغييرات قبل رفعها |
| 2 | `hotel_day_ledger` | سجل يومي محلي — يُحسب من الحجوزات |
| 3 | `auto_fix_runs` | سجلات AutoFix المحلية |
| 4 | `integrity_violations` | فحوصات السلامة المحلية |
| 5 | `app_sessions` | جلسات التطبيق المحلية |
| 6 | `restore_fix_log` | سجلات استعادة محلية |
| 7 | `sync_queue` | طابور المزامنة المحلي |
| 8 | `sync_conflicts` | تعارضات المزامنة المحلية |

### ⚠️ ملاحظة: `sync_logs` و `sync_state` و `app_users`

هذه الجداول **تُزامن جزئياً**:
- `sync_logs` — يُرفع للـ Cloud مباشرة (بدون outbox) لتتبع عمليات المزامنة
- `sync_state` — يُسحب من Cloud فقط (يخزن آخر طابع زمني للسحب)
- `app_users` — يُسحب من Cloud فقط (يخزن بيانات المستخدمين)

**لا تُعالج عبر outbox** — تُدار بطريقة منفصلة.

### ✅ الإصلاح المُطبّق

تمت إزالة الإدخال المُضلِّل `'outbox'` من `validFieldsPerCollection` لأنه:
- كان يحتوي على حقول لا تنتمي لجدول outbox الفعلي
- جدول outbox محلي فقط ولا يُزامن
- وجوده قد يُسبب التباساً للمطورين المستقبليين
