...
### سيناريو عملي: مزامنة فورية بعد الإضافة/الحذف/التعديل
1. عند نجاح أي عملية CRUD (زر إضافة، حذف، تعديل) في الطبقة UI يتم استدعاء `SyncManager.pushLocalChange` بنفس الجدول والصف والعملية.
2. الدالة تكتب الصف في `sync_queue` وتستدعي `_scheduleAutoSync()` لتجميع كل العمليات السريعة (نافذة 900ms مع حد أقصى 4 ثوانٍ).
3. إذا كانت الشبكة متاحة والبطارية فوق 20% يبدأ `_drainQueue()` فور انتهاء النافذة ويرفع التغييرات إلى Google Drive؛ وإلا توسم pending وتُنفذ تلقائياً عند تحسن الشرط (عودة الشبكة أو بدء الشحن).
4. بعد اكتمال الرفع يقوم `pullAndMerge()` بسحب أي تحديثات مقابلة لضمان اتساق الجهاز الحالي مع بقية الأجهزة.
5. النتيجة للمستخدم: الضغط على أزرار الإضافة/الحذف/التعديل يؤدي إلى مزامنة شبه فورية دون الحاجة لأي خطوات إضافية، حتى في حال الاتصال المتقطع يتم التعافي تلقائياً عند توفر الشروط.
