    ...
    for (final doc in docs) {
      ...
      if (entity == 'rooms') {
        ...
        if (existing == null) {
          ...
          await (db.update(db.rooms)..where((t) => t.id.equals(existing.id))).write(RoomsCompanion(
        ...
        if (existing == null) {
          ...
          await (db.update(db.bookings)..where((t) => t.id.equals(existing.id))).write(BookingsCompanion(
...
void main() {
  ...
  group('جهازين — مزامنة أساسية', () {
    test('الجهاز A يُنشئ غرفة ← push ← الجهاز B يسحبها', () async {
      ...
      await outboxA.merge(entity: 'rooms', op: 'create', localUuid: roomUuid, payload: {
        'roomNumber': '101', 'type': 'double', 'price': 200, 'status': 'شاغرة',
      }, clientTs: _epoch(), source: 'local');
    ...
    test('الجهاز A يُنشئ حجزاً ← push ← B يسحب ← B يُعدّل الغرفة', () async {
      ...
      await outboxA.merge(entity: 'rooms', op: 'create', localUuid: roomUuid, payload: {
        'roomNumber': '102', 'type': 'single', 'price': 150, 'status': 'شاغرة',
      }, clientTs: _epoch(), source: 'local');
      await outboxA.merge(entity: 'bookings', op: 'create', localUuid: bkgUuid, payload: {
        'roomNumber': '102', 'guestName': 'ضيف A',
      }, clientTs: _epoch(), source: 'local');
      ...
      await (dbB.update(dbB.rooms)..where((t) => t.id.equals(roomB.id))).write(RoomsCompanion(
        price: const d.Value(250), lastModified: d.Value(_epoch() + 100),
      ));
      await outboxB.merge(entity: 'rooms', op: 'update', localUuid: roomB.localUuid, payload: {
        'roomNumber': '102', 'type': 'single', 'price': 250, 'status': 'شاغرة',
      }, clientTs: _epoch() + 100, source: 'local');
    ...
    test('الجهازان ينشئان غرفاً بدون تعارض', () async {
      ...
      await outboxA.merge(entity: 'rooms', op: 'create', localUuid: 'room-indep-a', payload: {
        'roomNumber': '201', 'type': 'double', 'price': 300, 'status': 'شاغرة',
      }, clientTs: _epoch(), source: 'local');
      await dbB.into(dbB.rooms).insert(_room(number: '202', uuid: 'room-indep-b', type: 'suite', price: 500));
      await outboxB.merge(entity: 'rooms', op: 'create', localUuid: 'room-indep-b', payload: {
        'roomNumber': '202', 'type': 'suite', 'price': 500, 'status': 'شاغرة',
      }, clientTs: _epoch(), source: 'local');
  ...
  group('جهازين — كشف التعارضات وحلّها', () {
    test('تعديل سعر الغرفة — push from B wins (newerWins)', () async {
      ...
      await outboxA.merge(entity: 'rooms', op: 'create', localUuid: roomUuid, payload: {
        'roomNumber': '301', 'type': 'single', 'price': 100, 'status': 'شاغرة',
      }, clientTs: _epoch(), source: 'local');
      await simulatePush(dbA, outboxA, cloud, 'DeviceA');
      await simulatePull(dbB, outboxB, cloud);
      ...
      await (dbB.update(dbB.rooms)..where((t) => t.id.equals(roomB.id))).write(RoomsCompanion(
        price: const d.Value(350), lastModified: d.Value(_epoch() + 2000),
      ));
      await outboxB.merge(entity: 'rooms', op: 'update', localUuid: roomUuid, payload: {
        'roomNumber': '301', 'type': 'single', 'price': 350, 'status': 'شاغرة',
      }, clientTs: _epoch() + 2000, source: 'local');
    ...
    test('تعديل حقول مختلفة — الجهازان يتفقان بعد السحب', () async {
      ...
      await dbA.into(dbA.rooms).insert(_room(number: '401', uuid: roomUuid, type: 'single', price: 100, status: 'شاغرة'));
      await outboxA.merge(entity: 'rooms', op: 'create', localUuid: roomUuid, payload: {
        'roomNumber': '401', 'type': 'single', 'price': 100, 'status': 'شاغرة',
      }, clientTs: _epoch(), source: 'local');
      await simulatePush(dbA, outboxA, cloud, 'DeviceA');
      await simulatePull(dbB, outboxB, cloud);
      ...
      final roomB = await (dbB.select(dbB.rooms)..where((t) => t.localUuid.equals(roomUuid))).getSingle();
      await (dbB.update(dbB.rooms)..where((t) => t.id.equals(roomB.id))).write(RoomsCompanion(
        status: const d.Value('مشغولة'), lastModified: d.Value(_epoch() + 2000),
      ));
      await outboxB.merge(entity: 'rooms', op: 'update', localUuid: roomUuid, payload: {
        'roomNumber': '401', 'type': 'single', 'price': 100, 'status': 'مشغولة',
      }, clientTs: _epoch() + 2000, source: 'local');
    ...
    test('ConflictDetector يكتشف تعارض تعديل متزامن vectorClock', () async {
      final ancestor = {
        'roomNumber': '501', 'type': 'single', 'price': 150, 'status': 'شاغرة',
        'vectorClock': '{}', 'lastModified': 1000,
      };
      final local = {
        'roomNumber': '501', 'type': 'single', 'price': 250, 'status': 'شاغرة',
        'vectorClock': '{"deviceA": 2}', 'lastModified': 2000,
      };
      final remote = {
        'roomNumber': '501', 'type': 'single', 'price': 350, 'status': 'شاغرة',
        'vectorClock': '{"deviceB": 2}', 'lastModified': 2500,
      };
    ...
    test('SmartConflictResolver يحل تعارض السعر بـ newerWins', () async {
      final ancestor = {
        'roomNumber': '601', 'type': 'single', 'price': 150, 'status': 'شاغرة',
        'vectorClock': '{}', 'lastModified': 1000,
      };
      final local = {
        'roomNumber': '601', 'type': 'single', 'price': 200, 'status': 'شاغرة',
        'vectorClock': '{"deviceA": 2}', 'lastModified': 1000,
      };
      final remote = {
        'roomNumber': '601', 'type': 'single', 'price': 350, 'status': 'شاغرة',
        'vectorClock': '{"deviceB": 2}', 'lastModified': 2000,
      };
    ...
    test('دورة push/pull كاملة — 3 كيانات بدون فقدان', () async {
      ...
      await outboxB.merge(entity: 'rooms', op: 'create', localUuid: 'room-b-full', payload: {
        'roomNumber': '702', 'type': 'double', 'price': 300, 'status': 'شاغرة',
      }, clientTs: _epoch(), source: 'local');
      await simulatePush(dbB, outboxB, cloud, 'DeviceB');
      expect(cloud.getAll('rooms').length, 2);
  ...
  group('جهازين — حالات حافة', () {
    test('حذف من A — السحابة تعكس الحذف', () async {
      ...
      await outboxA.merge(entity: 'rooms', op: 'create', localUuid: roomUuid, payload: {
        'roomNumber': '801', 'type': 'single', 'price': 100, 'status': 'شاغرة',
      }, clientTs: _epoch(), source: 'local');
      await simulatePush(dbA, outboxA, cloud, 'DeviceA');

      await outboxA.merge(entity: 'rooms', op: 'delete', localUuid: roomUuid, payload: {
        'roomNumber': '801',
      }, clientTs: _epoch() + 100, source: 'local');
      await simulatePush(dbA, outboxA, cloud, 'DeviceA');
    ...
    test('100 عملية push/pull متتالية بين الجهازين', () async {
      ...
      for (var i = 0; i < count; i++) {
        ...
        await dbA.into(dbA.rooms).insert(_room(number: '9${i.toString().padLeft(2, '0')}', uuid: roomUuid, price: 100.0 + i));
        await outboxA.merge(entity: 'rooms', op: 'create', localUuid: roomUuid, payload: {
          'roomNumber': '9${i.toString().padLeft(2, '0')}', 'type': 'single', 'price': 100.0 + i, 'status': 'شاغرة',
        }, clientTs: _epoch(), source: 'local');
      }
      ...
      final roomsB = await dbB.select(dbB.rooms).get();
      for (var i = 0; i < count; i++) {
        expect(roomsB.where((r) => r.roomNumber == '9${i.toString().padLeft(2, '0')}').length, 1,
