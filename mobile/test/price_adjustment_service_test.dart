...
void main() {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        await db.into(db.bookingNights).insert(
          BookingNightsCompanion(
            ...
            nightEnd: Value('${_hotelDayKey(nightDate.add(const Duration(days: 1)))}T14:00:00'),
  ...
  group('تطبيق تغيير السعر (applyRoomPriceChange)', () {
    test('يُحدّث سعر الغرفة ويُنشئ outbox entries للحجز النشط', () async {
      ...
      final updatedRoom = await (db.select(db.rooms)
      ...
      expect(outboxEntries.any((e) => e.entity == 'rooms'), isTrue,
          reason: 'يجب إنشاء outbox entry لتحديث سعر الغرفة');
      expect(outboxEntries.any((e) => e.entity == 'price_adjustments'), isTrue,
    ...
    test('الحجوزات المغلقة (status=مغادر) لا تتأثر بتغيير السعر', () async {
      ...
      for (var i = 0; i < 4; i++) {
        ...
        final nightKey = _hotelDayKey(nightDate);
        final nextNightKey = _hotelDayKey(nightDate.add(const Duration(days: 1)));
        await db.into(db.bookingNights).insert(
          BookingNightsCompanion(
      ...
      final updatedRoom = await (db.select(db.rooms)
            ..where((r) => r.roomNumber.equals('202')))
          .getSingle();
  ...
  group('استعلامات تعديلات الأسعار', () {
    test('getAdjustmentsInDateRange يُرجع التعديلات في النطاق الصحيح', () async {
