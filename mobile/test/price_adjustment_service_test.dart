...
void main() {
  ...
  group('تعديل السعر أثناء الإقامة', () {
    test('السيناريو: دخول يوم 5 بسعر 10000، رفع السعر يوم 10 إلى 12000', () async {
        ...
        await db
            ...
            .insert(
              BookingNightsCompanion(
                ...
                nightEnd: Value('${nightDates[(i + 1) % nightDates.length]}T14:00:00'),
      ...
      final updatedBooking = await (db.select(db.bookings)..where((b) => b.id.equals(bookingId))).getSingle();
    ...
    test('تعديل السعر لا يؤثر على الحجوزات المغلقة', () async {
      ...
      final nights = await (db.select(db.bookingNights)..where((n) => n.bookingLocalId.equals(bookingId))).get();
