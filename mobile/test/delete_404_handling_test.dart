...
void main() {
  ...
  group('Delete 404 Handling Tests', () {
    ...
    test('deleteDocument يجب أن ينجح عند حذف مستند غير موجود (404)', () async {
      ...
      await expectLater(
        appwriteService.deleteDocument(collectionId: AppwriteConfig.roomsCollectionId, documentId: fakeDocumentId),
        completes,
      );
    ...
    test('deleteBooking يجب أن ينجح عند حذف حجز غير موجود', () async {
      ...
      await expectLater(appwriteService.deleteBooking(fakeBookingId), completes);
    }, timeout: const Timeout(Duration(seconds: 60)));

    test('deleteExpense يجب أن ينجح عند حذف مصروف غير موجود', () async {
      final fakeExpenseId = 'fake_expense_uuid_999';

      await expectLater(appwriteService.deleteExpense(fakeExpenseId), completes);
    }, timeout: const Timeout(Duration(seconds: 60)));

    test('deleteDocument يجب أن يرمي استثناء للأخطاء الأخرى (غير 404)', () async {
      await expectLater(
        appwriteService.deleteDocument(collectionId: 'invalid_collection_xyz', documentId: 'some_id'),
