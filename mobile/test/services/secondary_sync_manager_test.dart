...
void main() {
  ...
  group('SecondarySyncManager — Audit Fixes', () {
    test(
      'P0-2: maxAttempts = 10 (القيمة المتوقّعة)',
      () {
        expect(SecondarySyncManager.maxAttempts, 10,
            reason: 'maxAttempts يجب أن يكون 10 — إذا تغيّر يجب تحديث '
                'الاختبارات والـ UI الذي يعرض العداد');
      },
    );

    test(
      'P1-5: isCircuitOpen = false عند التهيئة (لا فشل بعد)',
      () {
        final mgr = SecondarySyncManager.instance;
        // بدون استدعاء sync()، الـ breaker يجب أن يكون مغلقاً
        expect(mgr.isCircuitOpen, isFalse,
            reason: 'الـ circuit breaker يجب أن يبدأ مغلقاً');
        expect(mgr.circuitOpenUntil, isNull,
            reason: 'لا يوجد وقت إغلاق قبل أي فشل');
      },
    );

    test(
      'P1-6: isSyncing = false عند التهيئة (لا توجد جلسة جارية)',
      () {
        final mgr = SecondarySyncManager.instance;
        expect(mgr.isSyncing, isFalse,
            reason: 'يجب أن يبدأ في حالة idle');
        expect(mgr.syncStartedAt, isNull,
            reason: 'لا يوجد وقت بدء قبل أي sync()');
      },
    );

    test(
      'P0-2: isPermanentError يصنّف 400/401/403 كأخطاء دائمة',
      () {
        ...
        expect(err404.code, 404,
            reason: '404 = not found = عابر (سيُعاد المحاولة)');
        expect(err500.code, 500,
    ...
    test(
      'P1-5: stopAutoSync يلغي المؤقّت',
      () {
        final mgr = SecondarySyncManager.instance;
        mgr.stopAutoSync();
        expect(mgr.isAutoSyncEnabled, isFalse,
    ...
    group('SecondarySyncResult', () {
      test('يحتوي على حقل dead (P0-2)', () {
        ...
        expect(result.dead, 1,
            reason: 'حقل dead ضروري لتتبّع السجلات التي انتقلت للحالة '
                'النهائية dead');
      });
  ...
  group('OutboxDao — Dead State (P0-2)', () {
    ...
    test('setDead, countDead, listDead, reviveFromDead methods موجودة', () {
      ...
      expect(true, isTrue, reason: 'compile-time check نجح — الـ methods '
          'موجودة في OutboxDao');
    });
  });
}
