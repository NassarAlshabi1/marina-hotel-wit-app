...
void main() {
  ...
  group('موظفين — CRUD', () {
    ...
    test('تعديل موظف', () async {
      ...
      // عدّل البيانات
      final rows = await empRepo.update(
        id,
        name: 'سالم علي أحمد',
        salary: 45000,
        position: 'مدير',
    ...
    test('تعديل موظف بـ localUuid', () async {
      ...
      final updated = await (db.select(db.employees)
    ...
    test('حذف موظف (soft delete)', () async {
      ...
      expect(active.where((e) => e.id == id), isEmpty,
          reason: 'الموظف المحذوف لا يجب أن يظهر في القائمة النشطة');
    });
  ...
  group('سحوبات الرواتب — CRUD', () {
    ...
    test('حفظ/تحديث سحب راتب (saveFromExpense upsert)', () async {
      ...
      // تحقق من الإنشاء
      final created = await (db.select(db.salaryWithdrawals)
            ..where((t) => t.reason.like('%exp_100%')
                & t.deletedAt.isNull()))
          .get();
      ...
      final after = await (db.select(db.salaryWithdrawals)
            ..where((t) => t.reason.like('%exp_100%')
                & t.deletedAt.isNull()))
          .get();
    ...
    test('حذف سحب راتب (soft delete)', () async {
      ...
      // تحقق من وجوده
      final before = await (db.select(db.salaryWithdrawals)
            ..where((t) => t.reason.like('%exp_200%')
                & t.deletedAt.isNull()))
          .get();
      ...
      final deleted = await (db.select(db.salaryWithdrawals)
            ..where((t) => t.reason.like('%exp_200%')))
          .get();
  ...
  group('الاستحقاقات — حساب', () {
    test('حساب استحقاق موظف بدون سحوبات', () async {
      ...
      expect(ent.netEntitlement, ent.totalEntitlement,
          reason: 'بدون سحوبات، المتبقي = إجمالي الاستحقاق');
    });
    ...
    test('حساب استحقاق بعد إضافة سحب راتب', () async {
      ...
      expect(ent.totalWithdrawals, 15000,
          reason: 'سحب راتب يجب أن يظهر في totalWithdrawals');
    ...
    test('حساب استحقاق بعد حذف السحب', () async {
      ...
      expect(entAfter.totalWithdrawals, 0,
          reason: 'بعد حذف المصروف، totalWithdrawals يجب أن يعود لـ 0');
    ...
    test('حساب استحقاق لكل الموظفين النشطين', () async {
      ...
      expect(entitlements.length, 2,
          reason: 'فقط الموظفون النشطون (غير المحذوفين) يجب أن يظهروا');
