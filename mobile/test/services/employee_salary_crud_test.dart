  ...
  group('موظفين — CRUD', () {
    test('إضافة موظف جديد', () async {
      ...
      expect(
        pending,
        greaterThan(0),
        reason: 'outbox يجب أن يحتوي على عملية create',
      );
    });
  ...
  group('سحوبات الرواتب — CRUD', () {
    ...
    test('حفظ/تحديث سحب راتب (saveFromExpense upsert)', () async {
      ...
      expect(
        after.length,
        1,
        reason: 'يجب تحديث السجل الموجود وليس إنشاء جديد',
      );
    ...
    test('حذف سحب راتب (soft delete)', () async {
      ...
      final deleted = await (db.select(
        db.salaryWithdrawals,
      )..where((t) => t.reason.like('%exp_200%'))).get();
