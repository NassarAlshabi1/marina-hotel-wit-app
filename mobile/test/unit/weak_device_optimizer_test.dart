...
void main() {
  group('WeakDeviceOptimizer classification', () {
    test('ذاكرة 1GB → جهاز ضعيف (optimizationLevel=2)', () {
      WeakDeviceOptimizer.instance.initialize(processorCount: 4, memoryMB: 1024);
    ...
    test('ذاكرة 2GB مع 2 أنوية → جهاز ضعيف', () {
      WeakDeviceOptimizer.instance.initialize(processorCount: 2, memoryMB: 2048);
      expect(WeakDeviceOptimizer.instance.isWeakDevice, isTrue);
    });

    test('ذاكرة 4GB مع 8 أنوية → جهاز عادي', () {
      WeakDeviceOptimizer.instance.initialize(processorCount: 8, memoryMB: 4096);
