...
void main() {
  ...
  group('Push: debtToRemote يحتوي على الحقول الأربعة', () {
    test(
      '1a. payload يحتوي على bookingUuidCache',
      () async {
        final debt = await _insertDebt(
          db,
          bookingUuidCache: 'booking-uuid-123',
        );
    ...
    test(
      '1b. payload يحتوي على debtorName',
      () async {
        final debt = await _insertDebt(
          db,
          debtorName: 'Ahmed Ali',
        );
        final payload = payloadMapper.debtToRemote(debt);
    ...
    test(
      '1f. payload لا يحتوي على bookingUuidCache عندما null/فارغ',
      () async {
    ...
    test(
      '1g. payload يحتوي على كل الحقول الأربعة معاً',
      () async {
        final debt = await _insertDebt(
          db,
  ...
  group('Pull: DebtsAdapter.fromJson يحفظ الحقول محلياً', () {
    test(
      '2a. fromJson يقرأ bookingUuidCache ويحفظه',
      () async {
        final json = {
          'localUuid': 'debt-1',
          'guestName': 'Guest 1',
    ...
    test(
      '2b. fromJson يقرأ debtorName ويحفظه',
      () async {
        final json = {
          'localUuid': 'debt-2',
          'guestName': 'Guest 2',
    ...
    test(
      '2c. fromJson يقرأ amount ويحفظه',
      () async {
        final json = {
          'localUuid': 'debt-3',
          'guestName': 'Guest 3',
    ...
    test(
      '2f. fromJson يتعامل مع القيم المفقودة (null بأمان)',
      () async {
        final json = {
          'localUuid': 'debt-6',
          'guestName': 'Guest 6',
  ...
  group('filterPayload: debts يسمح بالحقول الأربعة', () {
    test(
      '4a. filterPayload يحتفظ بـ bookingUuidCache',
      () {
        final payload = {
          'localUuid': 'x',
  ...
  group('Drive sync: DebtsAdapter.toJson يرسل الحقول الأربعة', () {
    test(
      '5a. toJson (drive) يحتوي على booking_uuid_cache',
      () async {
    ...
    test(
      '5b. toJson (drive) يحتوي على debtor_name',
      () async {
    ...
    test(
      '5c. toJson (drive) يحتوي على amount',
      () async {
    ...
    test(
      '5d. toJson (drive) يحتوي على date',
      () async {
    ...
    test(
      '5e. toJson (appwrite) يحتوي على الحقول بـ camelCase',
      () async {
        final debt = await _insertDebt(
          db,
  ...
  group('Round-trip: push → pull يحافظ على الحقول', () {
    test(
      '6a. push payload يمر عبر filterPayload بدون فقدان',
      () async {
        final debt = await _insertDebt(
          db,
        ...
        // 3. محاكاة pull: نمرر الـ payload عبر fromJson
        final companion = await debtsAdapter.resolveAndFromJson(
          db,
          filtered,
          src: Source.appwrite,
