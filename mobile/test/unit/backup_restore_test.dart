...
import 'package:marina_hotel_mobile/services/google_drive_backup_service.dart' show BackupFormat, BackupMetadata;
...
void main() {
  ...
  group('LocalBackupService — JSON format', () {
    test('createLocalBackup(json) produces valid gzip-compressed file', () async {
      await seedTestData();

      // إنشاء نسخة JSON محفوظة في backupDir المؤقت
      final backupPath = p.join(backupDir.path, 'marina_hotel_backup_test.json.gz');
      ...
      final metadata = BackupMetadata(
        ...
        totalRecords: roomsData.length + bookingsData.length + paymentsData.length + debtsData.length,
        deviceInfo: 'Test Device',
      );

      final backupData = <String, dynamic>{
        'metadata': metadata.toJson(),
        'rooms': roomsData.map((r) => r.toJson(serializer: lenientValueSerializer)).toList(),
        'bookings': bookingsData.map((b) => b.toJson(serializer: lenientValueSerializer)).toList(),
        'payments': paymentsData.map((p) => p.toJson(serializer: lenientValueSerializer)).toList(),
        'debts': debtsData.map((d) => d.toJson(serializer: lenientValueSerializer)).toList(),
        'booking_nights': nightsData.map((n) => n.toJson(serializer: lenientValueSerializer)).toList(),
        'employees': employeesData.map((e) => e.toJson(serializer: lenientValueSerializer)).toList(),
      };
      ...
      // ✅ التحقق: الملف موجود
      expect(await File(backupPath).exists(), isTrue, reason: 'backup file should exist');

      // ✅ التحقق: الحجم أصغر من JSON الخام (ضغط فعال)
      final compressedSize = (await File(backupPath).length());
      expect(compressedSize, lessThan(jsonBytes.length), reason: 'gzip compression should reduce size');
    ...
    test('round-trip JSON: insert → backup → restore → verify all data', () async {
      ...
      final backupPath = p.join(backupDir.path, 'marina_hotel_backup_roundtrip.json.gz');
      ...
      try {
        await db.transaction(() async {
          ...
          await insertList<dynamic>('rooms', (json) async {
            final data = Room.fromJson(Map<String, dynamic>.from(json), serializer: lenientValueSerializer);
            await db.into(db.rooms).insertOnConflictUpdate(data);
          });
          await insertList<dynamic>('employees', (json) async {
            final data = Employee.fromJson(Map<String, dynamic>.from(json), serializer: lenientValueSerializer);
            await db.into(db.employees).insertOnConflictUpdate(data);
          });
          await insertList<dynamic>('bookings', (json) async {
            final data = Booking.fromJson(Map<String, dynamic>.from(json), serializer: lenientValueSerializer);
            await db.into(db.bookings).insertOnConflictUpdate(data);
          });
          await insertList<dynamic>('payments', (json) async {
            final data = Payment.fromJson(Map<String, dynamic>.from(json), serializer: lenientValueSerializer);
            await db.into(db.payments).insertOnConflictUpdate(data);
          });
          await insertList<dynamic>('debts', (json) async {
            final data = Debt.fromJson(Map<String, dynamic>.from(json), serializer: lenientValueSerializer);
            await db.into(db.debts).insertOnConflictUpdate(data);
          });
          await insertList<dynamic>('booking_nights', (json) async {
            final data = BookingNight.fromJson(Map<String, dynamic>.from(json), serializer: lenientValueSerializer);
      ...
      expect(restoredRooms.single.roomNumber, equals(originalRoom.roomNumber));
      ...
      expect(restoredBookings.single.guestName, equals(originalBooking.guestName));
      expect(restoredBookings.single.roomNumber, equals(originalBooking.roomNumber));
      expect(restoredBookings.single.localUuid, equals(originalBooking.localUuid));

      expect(restoredPayments.single.amount, equals(originalPayment.amount));
      expect(restoredPayments.single.paymentMethod, equals(originalPayment.paymentMethod));

      expect(restoredDebts.single.totalAmount, equals(originalDebt.totalAmount));
      expect(restoredDebts.single.remainingAmount, equals(originalDebt.remainingAmount));

      expect(restoredNights.single.hotelDayKey, equals(originalNight.hotelDayKey));
      expect(restoredNights.single.nightlyRate, equals(originalNight.nightlyRate));
    ...
    test('restoring from a backup with newer DB version throws', () async {
      ...
      await File(backupPath).writeAsBytes(gzip.encode(utf8.encode(jsonEncode(backupData))));

      // ✅ محاكاة فحص الإصدار في _restoreFromJsonBackup
      final rawBytes = await File(backupPath).readAsBytes();
      final jsonData = jsonDecode(utf8.decode(gzip.decode(rawBytes))) as Map<String, dynamic>;
      final metaSource = jsonData['metadata'] as Map<String, dynamic>;
      final restoredMetadata = BackupMetadata.fromJson(Map<String, dynamic>.from(metaSource));
  ...
  group('SqliteBackupRestore — .db file format', () {
    test('SQLite header validation rejects non-SQLite files', () async {
      ...
      expect(isValidSqlite, isFalse, reason: 'non-SQLite file should fail header validation');
    });
    ...
    test('SQLite header validation accepts real SQLite file', () async {
      ...
      await sqfliteDb.execute('CREATE TABLE test (id INTEGER PRIMARY KEY, name TEXT)');
      ...
      expect(isValidSqlite, isTrue, reason: 'real SQLite file should pass header validation');
    });
    ...
    test('backup + restore round-trip preserves data via file copy', () async {
      ...
      await sqfliteDb.execute('CREATE TABLE rooms (id INTEGER PRIMARY KEY, number TEXT, price REAL)');
  ...
  group('Google Drive .db — SHA-256 hash verification', () {
    test('hash matches for intact file', () {
      final originalBytes = Uint8List.fromList(List.generate(1024, (i) => i % 256));
      ...
      expect(actualHash, equals(expectedHash), reason: 'hash should match for intact file');
    });
    ...
    test('size verification detects truncation', () {
      ...
      expect(actualSize, isNot(equals(expectedSize)), reason: 'truncated file should fail size check');
      expect(actualSize < expectedSize, isTrue);
    });

    test('appProperties carry hash and size metadata correctly', () {
      final originalBytes = Uint8List.fromList(List.generate(2048, (i) => i % 256));
  ...
  group('Atomicity & rollback', () {
    test('failed restore does not corrupt existing data (atomic transaction)', () async {
          ...
          await db
              ...
              .insert(
                RoomsCompanion(
                  ...
                  lastModified: d.Value(DateTime.now().millisecondsSinceEpoch),
                ),
              );
      ...
      expect(roomsAfterFailedRestore.single.roomNumber, equals('101'), reason: 'original room 101 should still exist');
    });

    test('file replacement via temp + rename preserves data on success', () async {
      ...
      await originalDb.execute('CREATE TABLE test (id INTEGER PRIMARY KEY, value TEXT)');
      ...
      await backupDb.execute('CREATE TABLE test (id INTEGER PRIMARY KEY, value TEXT)');
    ...
    test('rollback to .pre_restore on rename failure', () async {
      ...
      expect(await File(preRestorePath).exists(), isTrue, reason: 'pre_restore file should exist for rollback');
      expect(await File(dbPath).exists(), isFalse, reason: 'main db file should not exist yet');
      ...
      await restoredDb.close();
      expect(rows.length, equals(1));
      expect(rows[0]['value'], equals('original'), reason: 'rollback should restore original data');
    });
  });
  ...
  group('BackupMetadata serialization', () {
    ...
    test('falls back to JSON format for unknown format names', () {
      ...
      final restored = BackupMetadata.fromJson(json);
      expect(restored.format, equals(BackupFormat.json), reason: 'unknown format name should fall back to json');
    });
  ...
  group('WAL mode integrity (production-critical)', () {
    ...
    Future<String> _createWalDatabase(String path, {required int rowCount}) async {
      final database = await openDatabase(
        path,
        version: 1,
        onCreate: (db, _) async {
          await db.execute('CREATE TABLE rooms (id INTEGER PRIMARY KEY, number TEXT, price REAL)');
        },
      );
    ...
    test('WAL checkpoint before backup is essential — bytes without checkpoint may miss recent data', () async {
      ...
      final noCheckpointCopyPath = p.join(tempDir.path, 'no_checkpoint_copy.db');
      ...
      try {
        final copyDb = await openDatabase(noCheckpointCopyPath, readOnly: true);
        noCheckpointCount = Sqflite.firstIntValue(await copyDb.rawQuery('SELECT COUNT(*) FROM rooms')) ?? 0;
        await copyDb.close();
      } catch (e) {
      ...
      final checkpointCopyPath = p.join(tempDir.path, 'with_checkpoint_copy.db');
      ...
      final withCheckpointCount = Sqflite.firstIntValue(await copyDb2.rawQuery('SELECT COUNT(*) FROM rooms')) ?? 0;
      await copyDb2.close();
      ...
      expect(
        withCheckpointCount,
        equals(100),
        reason: 'after WAL checkpoint, all 100 records should be in the .db file',
      );
      ...
      debugPrint('📊 without checkpoint: $noCheckpointCount records, with checkpoint: $withCheckpointCount records');
    ...
    test('restoreDatabase MUST delete stale -wal/-shm files (regression test)', () async {
      ...
      expect(rows.length, equals(1), reason: 'restored DB should have only new_data');
      expect(
        rows[0]['value'],
        equals('new_data_only'),
        reason: 'restored DB should contain only new data, not stale old data',
      );
    ...
    test('restoreDatabase without -wal cleanup can leak stale data (proves the bug exists)', () async {
      ...
      debugPrint('📊 -wal exists after close: $walExisted, size: ${walExisted ? await walFile.length() : 0}');
      ...
      debugPrint('📊 rows after restore without -wal cleanup: ${rows.length}');
  ...
  group('deleteLocalBackup sidecar cleanup', () {
    test('deleting a .sqlite backup should also delete its .metadata.json', () async {
      // محاكاة ملفات النسخة الاحتياطية SQLite
      final sqlitePath = p.join(backupDir.path, 'marina_hotel_backup_test.sqlite');
      final metadataPath = p.join(backupDir.path, 'marina_hotel_backup_test.metadata.json');

      await File(sqlitePath).writeAsBytes([0x53, 0x51, 0x4c, 0x69]); // SQLite header
      ...
      expect(await File(sqlitePath).exists(), isFalse, reason: 'sqlite file should be deleted');
      expect(await File(metadataPath).exists(), isFalse, reason: 'metadata file should also be deleted');
    });

    test('current deleteLocalBackup leaves orphaned .metadata.json (proves the bug)', () async {
      // توثيق الخطأ الحالي: deleteLocalBackup يحذف .sqlite فقط دون .metadata.json
      final sqlitePath = p.join(backupDir.path, 'marina_hotel_backup_orphan.sqlite');
      final metadataPath = p.join(backupDir.path, 'marina_hotel_backup_orphan.metadata.json');
