...
void main() {
  ...
  group('P0-D: outbox coalescing - delivery state reset on payload/op change', () {
    test('merge with changed payload resets delivered_to_primary to false', () async {
    ...
    test('resolveConflict resets both delivery flags to false', () async {
      await outboxDao.merge(
        entity: 'rooms',
        op: 'create',
      ...
      expect(row.read<String>('processing_status'), equals('pending'));
    });
  });

  group('P0-E: hard delete vs tombstone - cleanupForMissingEntities safety', () {
    test('cleanupForMissingEntities does NOT delete pending entries', () async {
  ...
  group('P0-H: reclaimAllStuckProcessingOnStartup', () {
    test('reclaims all processing entries regardless of age', () async {
      await outboxDao.merge(
        entity: 'rooms',
        op: 'create',
  ...
  group('P0-F: SmartConflictResolver end-to-end resolution', () {
    ...
    test('noConflictRemoteNewer does NOT flag for re-upload', () {
      final localData = {
        'local_uuid': 'room-003',
        'room_number': '103',
        'price': 100,
  ...
  group('P0-F/P0-D integration: vector clock + outbox delivery flag reset', () {
    test('vector clock concurrent detection works correctly', () {
    ...
    test('vector clock sequential detection works correctly', () {
