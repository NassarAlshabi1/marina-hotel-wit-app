...
void main() {
  group('RestoreFixService', () {
    ...
    test('snapshot rollback restores data on failure', () async {
      ...
      final failingService = RestoreFixService(database,
    ...
    test('end-to-end auto fix normalizes bookings and rooms', () async {
      ...
      final updatedBooking = await (database.select(database.bookings)
          ...
          .getSingle();
      final expectedNights = Time.nightsWithCutoff(DateTime(2024, 8, 10, 16),
  ...
  group('DeltaSyncService', () {
    ...
    test('detects inserts and normalizes timestamps', () async {
      ...
      expect(
          computation.changes
              .any((c) => c.entity == 'rooms' && c.operation == 'insert'),
          isTrue);
    ...
    test('detects updates after mirror persistence', () async {
      ...
      await (database.into(database.syncState))
          .insertOnConflictUpdate(SyncStateCompanion(
        id: const Value(1),
      ...
      expect(
          secondComputation.changes
              .any((c) => c.entity == 'rooms' && c.operation == 'update'),
          isTrue);
    });
    ...
    test('detects deletes after soft removal', () async {
      ...
      expect(
          deleteComputation.changes
              .any((c) => c.entity == 'rooms' && c.operation == 'delete'),
          isTrue);
    });
  });
}
