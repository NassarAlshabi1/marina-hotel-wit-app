...
void main() {
  testWidgets('showSyncSuccess builds overlay with correct text', (tester) async {
    final key = GlobalKey();
  ...
  testWidgets('showSyncError builds overlay and retry button works', (tester) async {
    final key = GlobalKey();
    var retried = false;
    await tester.pumpWidget(MaterialApp(home: Scaffold(key: key)));
    SyncNotificationManager.showSyncError(key.currentContext!, error: 'fail', onRetry: () => retried = true);
    await tester.pump();
