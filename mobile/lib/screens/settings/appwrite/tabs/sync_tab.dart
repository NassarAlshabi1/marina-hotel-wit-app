...
class _AppwriteSyncTabState extends ConsumerState<AppwriteSyncTab> {
  ...
  List<SyncErrorRecord> _syncErrors = [];
  Timer? _errorsRefreshTimer;
  ...
  void initState() {
    ...
    // ⭐ تحديث الأخطاء كل 10 ثواني
    _errorsRefreshTimer = Timer.periodic(const Duration(seconds: 10), (_) {
      _loadSyncErrors();
    });
  }
  ...
  @override
  void dispose() {
    _errorsRefreshTimer?.cancel();
    super.dispose();
  }
  
  /// ⭐ تحميل أخطاء المزامنة
  void _loadSyncErrors() {
    final errors = AppwriteDeltaSync.instance.syncErrors;
    if (mounted && errors.length != _syncErrors.length) {
      setState(() {
        _syncErrors = List.from(errors);
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        // ⭐ قسم الأخطاء الفاشلة (يظهر فقط إذا وجدت أخطاء)
        if (_syncErrors.isNotEmpty) ...[
          _buildFailedRecordsCard(),
          const SizedBox(height: UIConstants.spacingLG),
        ],
  ...
  /// ⭐ بناء بطاقة السجلات الفاشلة
  Widget _buildFailedRecordsCard() {
    return Card(
      elevation: 2,
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'سجلات فاشلة (${_syncErrors.length})',
                ...
                // زر إعادة محاولة الكل
                if (_syncErrors.isNotEmpty)
                  TextButton.icon(
                    onPressed: _isRetrying ? null : _retryAllFailed,
                    icon: _isRetrying
                        ? const SizedBox(
                            width: 16,
                            height: 16,
                    ...
                    label: Text(_isRetrying ? 'جاري...' : 'إعادة الكل'),
          ...
          // قائمة الأخطاء (أول 5 فقط)
          ..._syncErrors.take(5).map((error) => _buildErrorItem(error)),
          
          // إذا كان هناك أكثر من 5 أخطاء
          if (_syncErrors.length > 5)
            Padding(
              ...
              child: Text(
                '... و ${_syncErrors.length - 5} سجلات أخرى',
                style: TextStyle(
              ...
              children: [
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: _showAllErrorsDialog,
  ...
  /// ⭐ بناء عنصر خطأ واحد
  Widget _buildErrorItem(SyncErrorRecord error) {
    return Container(
      padding: const EdgeInsets.symmetric(
        horizontal: UIConstants.spacingMD,
        vertical: UIConstants.spacingSM,
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: _getOperationColor(error.operation).withOpacity(0.1),
  ...
  Future<void> _retryAllFailed() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('خطأ في إعادة المحاولة: $e'),
  ...
  /// ⭐ إعادة محاولة سجل واحد
  Future<void> _retrySingleError(SyncErrorRecord error) async {
    try {
      // إعادة محاولة جميع السجلات الفاشلة (لتبسيط الكود)
      final result = await AppwriteDeltaSync.instance.pushDeltaChanges(retryFailed: true);
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              result.success
                  ? 'تم إعادة محاولة السجلات الفاشلة بنجاح'
                  : 'فشلت إعادة المحاولة: ${result.message}',
            ),
  ...
  /// ⭐ عرض جميع الأخطاء في نافذة منفصلة
      ...
      builder: (context) => AlertDialog(
        title: Row(
          children: [
            ...
            Text('جميع السجلات الفاشلة (${_syncErrors.length})'),
          ],
        ),
        content: SizedBox(
          ...
          child: ListView.builder(
            itemCount: _syncErrors.length,
            itemBuilder: (context, index) {
              final error = _syncErrors[index];
              return Card(
        ...
        actions: [
          ...
          ElevatedButton.icon(
            ...
            icon: const Icon(Icons.refresh),
            label: const Text('إعادة محاولة الكل'),
  ...
  Future<void> _clearAllErrors() async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('مسح جميع الأخطاء'),
        content: const Text(
          'هل تريد مسح جميع سجلات الأخطاء؟\n\n'
          '⚠️ لن تتمكن من إعادة محاولة هذه السجلات بعد المسح.',
        ),
        actions: [
          TextButton(
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('تم مسح جميع الأخطاء'),
