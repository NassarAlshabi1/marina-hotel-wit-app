...
class _GuestEditScreenState extends ConsumerState<GuestEditScreen> {
  ...
  Future<void> _endPriceAdjustment(String adjustmentUuid, String type) async {
    ...
    try {
      ...
      // ✅ تحديث: تصفير قيمة الخصم في جدول الحجوزات عند إلغاء التخفيض لضمان المزامنة الصحيحة
      for (final booking in widget.guest.bookings) {
        await ref.read(bookingsRepoProvider).update(
          booking.id,
          discount: 0,
          discountType: 'per_night',
        );
        
        // تحديث المتحكمات المحلية لتعكس التغيير في الواجهة
        if (_discountControllers.containsKey(booking.id)) {
          _discountControllers[booking.id]!.text = '0';
        }
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('تم إنهاء $type وتصفير القيمة بنجاح'),
          backgroundColor: Colors.green,
        ),
      );
    } catch (e) {
