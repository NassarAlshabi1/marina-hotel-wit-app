...
class _ComprehensiveBackupScreenState
          ...
          Container(
            ...
            child: TabBar(
              ...
              tabs: const [
                ...
                Tab(icon: Icon(Icons.import_export), text: 'إدارة الملفات'),
              ],
            ),
          ),
            ...
            if (state.hasStoragePermission) ...[
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(
                    state.status == BackupStatus.checkingPermissions
                        ? 'جاري التحقق...'
                        : 'طلب الأذونات',
                  ),
                  style: ElevatedButton.styleFrom(
  ...
  void _showRestoreConfirmation(String identifier, BackupType type) {
    ...
    if (type == BackupType.googleDrive) {
      ...
      recordsCount = int.tryParse(
        backup.metadata?['records_count']?.toString() ?? '0',
      );
    } else {
      try {
        backup = ref
          ...
          ElevatedButton(
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(
                    'جارٍ دمج ${jsonBackups.length} نسخة احتياطية...',
                  ),
              ...
              children: [
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      final backup = state.localBackups[index];
                      final isSelected = selectedBackups.contains(
                        backup.filePath,
                      );
