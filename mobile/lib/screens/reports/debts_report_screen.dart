...
class _DebtsReportScreenState extends ConsumerState<DebtsReportScreen> {
  ...
  Future<void> _fetchReport() async {
    ...
    try {
      ...
      final cachedResult = await ReportCacheService.instance.get<
          _DebtsReportResult>(
        ReportCacheKeys.debts,
        _getCacheParams(),
        (json) => _DebtsReportResult.fromJson(json),
      );
      ...
      if (cachedResult != null) {
        setState(() {
          ...
          _totalDebt = cachedResult.totalDebt;
          _totalPaid = cachedResult.totalPaid;
          _totalRemaining = cachedResult.totalRemaining;
          _isFromCache = true;
        });
        return;
      }
  ...
  Future<void> _exportPdf() async {
    ...
    final statusLabel = switch (_statusFilter) {
      DebtStatusFilter.all => 'الكل',
      DebtStatusFilter.settled => 'مسدد',
      DebtStatusFilter.pending => 'غير مسدد',
    };
                ...
                Row(
                  children: [
                    ...
                    Expanded(
                      ...
                      child: SizedBox(
                        height: inputsHeight,
                        child: DropdownButtonFormField<DebtStatusFilter>(
                          value: _statusFilter,
                          ...
                          items: const [
                            ...
                            DropdownMenuItem(
                              value: DebtStatusFilter.settled,
                              child: Text('مسدد', style: TextStyle(fontSize: 12)),
                            ),
                            DropdownMenuItem(
                              value: DebtStatusFilter.pending,
                              child: Text('غير مسدد', style: TextStyle(fontSize: 12)),
                            ),
                          ],
                          onChanged: (value) {
                            setState(() {
                              _statusFilter = value ?? DebtStatusFilter.all;
                            });
                            _fetchReport();
                          },
                        ),
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Row(
                  children: [
                    _buildSummaryItem('الدين', _totalDebt, Colors.black),
                    const SizedBox(width: 12),
                    _buildSummaryItem('المدفوع', _totalPaid, Colors.green),
                    const SizedBox(width: 12),
...
class _DebtProcessParams {
  ...
  factory _DebtProcessParams.fromJson(Map<String, dynamic> json) =>
      _DebtProcessParams(
        ...
        statusFilter: DebtStatusFilter.values.firstWhere(
          (e) => e.name == json['statusFilter'],
          orElse: () => DebtStatusFilter.all,
        ),
      );
}
...
class _DebtsReportResult {
  ...
  factory _DebtsReportResult.fromJson(Map<String, dynamic> json) =>
      _DebtsReportResult(
        rows: (json['rows'] as List)
            .map((r) => _DebtReportRow.fromJson(r as Map<String, dynamic>))
            .toList(),
...
class _DebtReportRow {
  ...
  factory _DebtReportRow.fromJson(Map<String, dynamic> json) =>
      _DebtReportRow(
...
_DebtsReportResult _processDebtsData(_DebtProcessParams params) {
  ...
  for (final data in params.debts) {
    ...
    switch (params.statusFilter) {
      case DebtStatusFilter.settled:
        if (!isSettled) continue;
      case DebtStatusFilter.pending:
        if (isSettled) continue;
