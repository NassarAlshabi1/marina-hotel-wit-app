...
class _ExpensesListScreenState extends ConsumerState<ExpensesListScreen>
  ...
  static const String _salaryAdvanceAction = ExpenseTypes.salaryAdvance;
  ...
  List<String> get _expenseTypes {
    ...
    return types.where((t) => t != ExpenseTypes.salaryAdvance).toList();
  }
  ...
  String _hotelDayKeyFromDate(DateTime date) {
    return HotelTimeEngine.getHotelDayKey(
        dateTime: DateTime(date.year, date.month, date.day, 
            HotelTimeEngine.boundaryHour, 0, 1));
  }
  ...
  Future<void> _pickDate({required bool isFrom}) async {
    ...
    setState(() {
      ...
      if (isFrom) {
        ...
        _fromDate = DateTime(picked.year, picked.month, picked.day, HotelTimeEngine.boundaryHour, 0, 0);
            ...
            actions: [
              ...
              FilledButton(
                onPressed: () {
                  ...
                  // ✅ التحقق من صحة المبلغ قبل إغلاق الحوار
                  final parsedAmt = CurrencyFormatter.parseAmount(amount.text) ?? 0;
                  if (parsedAmt <= 0) {
                    ScaffoldMessenger.of(ctx).showSnackBar(
                      SnackBar(
  ...
  Future<void> _deleteExpense(Expense expense) async {
    ...
    try {
      ...
      // ✅ حذف سحب الراتب أولاً ثم المصروف — لحماية تكامل البيانات
