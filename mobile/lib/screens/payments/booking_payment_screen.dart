...
class _BookingPaymentScreenState extends ConsumerState<BookingPaymentScreen>
  ...
  Future<void> _sendPaymentConfirmation(double amountPaidNow, double remaining, String cleanedPhone) async {
    ...
    final message = StringBuffer()
      ...
      ..writeln('تم استلام دفعتك بقيمة ${formatAmount(amountPaidNow)} ر.س')
      ..writeln('رقم الغرفة: ${widget.booking.roomNumber}')
      ..writeln('المبلغ المتبقي: ${formatAmount(remaining)} ر.س')
        ...
        content: Column(
          ...
          children: [
            Container(
              ...
              child: Column(
                children: [
                  Text('المبلغ: ${_currencyFmt.format(amount)} ر.س'),
                  Text('عدد الليالي: $nights'),
                  Text('سعر الليلة: ${_currencyFmt.format(amount / nights)} ر.س'),
                ],
              ),
            ),
            const SizedBox(height: 16),
  ...
  Future<void> _processDailyPayment(double amount, String notes, int nights) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('تم تسجيل دفع $nights ${nights == 1 ? 'ليلة' : 'ليالي'} إضافية - ${_currencyFmt.format(amount)} ر.س'),
  ...
  Future<void> _sendExtendedStayPaymentConfirmation(
    ...
    String message = 'عزيزي ${widget.booking.guestName}، تم استلام دفعة بقيمة: ${formatAmount(amountPaidNow)} ر.س\n';
    message += 'رقم الغرفة: ${widget.booking.roomNumber}\n';
    message += 'دفع $nightsPaid ${nightsPaid == 1 ? 'ليلة إضافية' : 'ليالي إضافية'}\n';
    message += 'المبلغ المتبقي: ${formatAmount(remaining)} ر.س\n';
  ...
  Future<void> _performSendAccountStatement(BookingPaymentSummary summary) async {
    ...
    statement += '\nتفاصيل الحساب:\n';
    statement += 'المبلغ الإجمالي: ${_currencyFmt.format(summary.totalAmount)} ر.س\n';
    statement += 'المبلغ المدفوع: ${_currencyFmt.format(summary.paidAmount)} ر.س\n';
    statement += 'المبلغ المتبقي: ${_currencyFmt.format(summary.remainingAmount)} ر.س\n\n';
    ...
    if (summary.payments.isNotEmpty) {
      ...
      for (int i = 0; i < summary.payments.length; i++) {
        ...
        statement += '${i + 1}. ${_currencyFmt.format(payment.amount)} ر.س - ${payment.method.displayName} - $paymentDate\n';
      }
      statement += '\n';
    }
  ...
  Future<void> _sendPaymentReminder(BookingPaymentSummary summary) async {
    ...
    reminder += 'المبلغ الإجمالي: ${_currencyFmt.format(summary.totalAmount)} ر.س\n';
    reminder += 'المبلغ المدفوع: ${_currencyFmt.format(summary.paidAmount)} ر.س\n';
    reminder += 'المبلغ المتبقي: ${_currencyFmt.format(summary.remainingAmount)} ر.س\n\n';
                ...
                return Column(
                  ...
                  children: [
                    Container(
                      ...
                      child: Column(
                        children: [
                          Text('سعر الليلة: ${_currencyFmt.format(roomRate)} ر.س'),
                          Text('عدد الليالي: $nights'),
                          Text(
                            'التكلفة الإجمالية: ${_currencyFmt.format(totalCost)} ر.س',
                            style: const TextStyle(fontWeight: FontWeight.bold),
                          ),
                        ],
                      ),
  ...
  Future<void> _sendExtensionConfirmation(
    ...
    message += 'المبلغ المدفوع: ${formatAmount(amount)} ر.س\n';
