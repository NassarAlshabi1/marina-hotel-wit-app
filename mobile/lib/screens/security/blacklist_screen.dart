...
class _BlacklistScreenState extends ConsumerState<BlacklistScreen> {
  ...
  void _openEntryDialog(
    ...
    final nationalityCtrl =
        TextEditingController(text: isEdit ? (entry.nationality ?? '') : '');
    final nationalIdCtrl =
        TextEditingController(text: isEdit ? (entry.nationalId ?? '') : '');
    final phoneCtrl =
        TextEditingController(text: isEdit ? (entry.phone ?? '') : '');
    final reasonCtrl =
        TextEditingController(text: isEdit ? (entry.reason ?? '') : '');
    final notesCtrl =
            ...
            onPressed: () async {
              ...
              try {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(
                      isEdit ? 'تم تعديل: ${nameCtrl.text}' : 'تمت الإضافة: ${nameCtrl.text}',
                    ),
