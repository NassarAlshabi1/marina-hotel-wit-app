...
class SyncManager {
    ...
    try {
      ...
      try {
        ...
        if (!force &&
            remoteResult != null &&
            remoteResult.metadata.lastDeviceId == deviceId &&
            await compareChecksumAsync(remoteResult.snapshot)) {
  ...
  Future<SyncMergeResult> _mergeSnapshotsStreaming({
    ...
    for (final table in allTableNames) {
      ...
      final localMap = <String, Map<String, dynamic>>{};
      int recordCount = 0;
      await for (final batch in _streamTableRows(table,
          batchSize: SyncConfig.streamingBatchSize)) {
      ...
      debugPrint(
          '  📊 $table: $recordCount سجلات محلية، ${remoteMap.length} سجلات بعيدة');
    ...
    for (final key in allKeys) {
      ...
      if (remoteDeleted != null || localDeleted != null) {
        final remoteTs = remoteDeleted ??
            DateTime.fromMillisecondsSinceEpoch(0, isUtc: true);
        final localTs =
            localDeleted ?? DateTime.fromMillisecondsSinceEpoch(0, isUtc: true);
        if (localTs.isAfter(remoteTs)) {
          winner = localRow;
          operation = 'delete';
        } else {
        ...
        final localUpdatedTs =
            localUpdated ?? DateTime.fromMillisecondsSinceEpoch(0, isUtc: true);
        if (localUpdatedTs.isAfter(remoteUpdatedTs)) {
          winner = localRow;
          operation = 'update';
        } else if (remoteUpdatedTs.isAfter(localUpdatedTs)) {
