...
class AppwriteDeltaSync {
  ...
  Future<void> _applyBookingChange(
    ...
    final companion = BookingsCompanion(
      ...
      stayDurationIso: _nullableValue<String>(_asString(data['stayDurationIso'])),
      lastNightEpoch: _nullableValue<int>(_asInt(data['lastNightEpoch'])),
      isOverdue: d.Value(_asBool(data['isOverdue']) ?? false),
      isFullyPaid: d.Value(_asBool(data['isFullyPaid']) ?? false),
      remainingBalanceCached: d.Value(_asDouble(data['remainingBalanceCached'])),
      totalDueCached: d.Value(_asDouble(data['totalDueCached'])),
      totalPaidCached: d.Value(_asDouble(data['totalPaidCached'])),
      needsCheckoutReview: d.Value(_asBool(data['needsCheckoutReview']) ?? false),
      financialHash: _nullableValue<String>(_asString(data['financialHash'])),
      financialFrozenAt: _nullableValue<String>(_asString(data['financialFrozenAt'])),
    );
  ...
  static const _syncFields = <String>{
    'localUuid',
    'serverId',
    'createdAt',
    'updatedAt',
  ...
  static final _allowedFields = <String, Set<String>>{
    'rooms': {'basePrice', 'bedsCount', 'cleaningStatus', 'features', 'floor', 'idempotencyKey', 'imageUrl', 'lastCleanedHotelDay', 'lastCleaningTime', 'lastOccupiedHotelDay', 'price', 'requiresMaintenance', 'roomNumber', 'roomType', 'status', 'type'},
    'bookings': {'actualCheckout', 'calculatedNights', 'checkinDate', 'checkoutDate', 'discount', 'discountStartDate', 'discountType', 'expectedNights', 'financialFrozenAt', 'financialHash', 'guestAddress', 'guestEmail', 'guestIdIssueDate', 'guestIdIssuePlace', 'guestIdNumber', 'guestIdType', 'guestName', 'guestNationality', 'guestPhone', 'hotelDayCheckin', 'hotelDayCheckout', 'idempotencyKey', 'isFullyPaid', 'isOverdue', 'lastNightEpoch', 'needsCheckoutReview', 'notes', 'remainingBalanceCached', 'roomNumber', 'serverBookingId', 'status', 'stayDurationIso', 'totalDueCached', 'totalNightsCached', 'totalPaidCached'},
    'booking_notes': {'alertType', 'alertUntil', 'bookingId', 'idempotencyKey', 'isActive', 'noteText'},
    'booking_nights': {'adjustment', 'appliedAdjustmentUuid', 'appliedAdjustmentsJson', 'baseRate', 'bookingLocalId', 'finalRate', 'hotelDayKey', 'idempotencyKey', 'isProcessedByAutoFix', 'nightEnd', 'nightStart', 'nightlyRate', 'sequence'},
    'employees': {'basicSalary', 'hireDate', 'idempotencyKey', 'name', 'phone', 'position', 'status'},
    'expenses': {'amount', 'cashFlowUuid', 'cashTransactionId', 'categoryUuid', 'date', 'description', 'expenseType', 'hotelDayKey', 'idempotencyKey', 'isAutoGenerated', 'relatedId'},
    'cash_transactions': {'amount', 'createdBy', 'description', 'idempotencyKey', 'referenceId', 'referenceType', 'registerId', 'transactionTime', 'transactionType'},
    'payments': {'amount', 'bookingLocalId', 'bookingUuidCache', 'cashTransactionLocalId', 'cashTransactionServerId', 'hotelDayKey', 'idempotencyKey', 'isImmutable', 'isPendingBalance', 'isVoided', 'linkedDebtUuid', 'notes', 'paymentDate', 'paymentMethod', 'referenceNumber', 'revenueType', 'roomNumber', 'serverBookingId', 'serverPaymentId', 'voidedAt', 'voidedBy'},
    'debts': {'bookingLocalId', 'checkinDate', 'checkoutDate', 'dateRecorded', 'debtReason', 'debtUuid', 'guestName', 'hotelDayClosed', 'hotelDayOpened', 'idempotencyKey', 'isFromAutoFix', 'isSettled', 'note', 'paidAmount', 'paymentDate', 'pledge', 'pledgeType', 'remainingAmount', 'settlementConfirmed', 'totalAmount'},
    'hotel_day_ledger': {'bookingsProcessed', 'debtsProcessed', 'expensesProcessed', 'hotelDayKey', 'idempotencyKey', 'occupancyRate', 'paymentsProcessed', 'pendingBalances', 'status', 'totalExpenses', 'totalIncome'},
    'shift_notes': {'content', 'createdBy', 'expiresAt', 'idempotencyKey', 'isRead', 'priority', 'shiftType', 'title'},
    'salary_cycles': {'actualPaid', 'cycleKey', 'employeeId', 'expectedAmount', 'hotelDayEnd', 'hotelDayStart', 'idempotencyKey', 'remainingAmount', 'status'},
    'salary_payments': {'amount', 'cycleId', 'employeeId', 'hotelDayKey', 'idempotencyKey', 'isAutoGenerated', 'method', 'notes', 'paymentDate', 'paymentDateIso'},
    'price_adjustments': {'adjustmentType', 'appliedBy', 'effectiveDate', 'hotelDayKey', 'isReversed', 'newValue', 'previousValue', 'reason', 'reversedAt', 'reversedBy', 'targetType', 'targetUuid'},
    'booking_price_adjustments': {'adjustmentMode', 'adjustmentType', 'amount', 'appliedBy', 'bookingLocalId', 'bookingLocalUuid', 'cancelledAt', 'cancelledBy', 'effectiveHotelDay', 'endHotelDay', 'isActive', 'reason'},
    'audit_logs': {'amountImpact', 'changedFields', 'entityId', 'entityType', 'entityUuid', 'hotelDayKey', 'ipAddress', 'isFinancial', 'operationType', 'performedBy', 'previousState', 'timestamp', 'timestampIso'},
    'payment_voids': {'approvedBy', 'bookingUuid', 'hotelDayKey', 'originalPaymentId', 'originalPaymentUuid', 'reversalPaymentUuid', 'voidReason', 'voidedAmount', 'voidedAt', 'voidedAtIso', 'voidedBy'},
  };
  ...
  Map<String, dynamic> _sanitizePayload(
    ...
    final allowed = collectionEntity != null
        ? _allowedFields[collectionEntity]
        : null;
    ...
    payload.forEach((key, value) {
      ...
      if (allowed != null &&
          !allowed.contains(camelKey) &&
          !_syncFields.contains(camelKey)) {
        return;
      }
  ...
  Future<int> _getLastDeltaPushTimestamp() async {
    final prefs = await SharedPreferences.getInstance();
    final value = prefs.getInt(_prefsLastDeltaPushKey);
    if (value != null) return value;
    return prefs.getInt('appwrite_last_delta_sync') ?? 0;
  }

  Future<int> _getLastDeltaPullTimestamp() async {
    final prefs = await SharedPreferences.getInstance();
    final value = prefs.getInt(_prefsLastDeltaPullKey);
    if (value != null) return value;
    return prefs.getInt('appwrite_last_delta_sync') ?? 0;
  }
