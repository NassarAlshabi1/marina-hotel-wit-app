...
class AppwriteDeltaSync {
  ...
  Future<void> _pushSingleChange(DeltaSyncChange change) async {
    ...
    // ⭐ إضافة الحقول المطلوبة لـ Appwrite (snake_case)
    payload['device_id'] = _deviceId;
    payload['sync_timestamp'] = Time.nowEpoch();

    // ⭐ التأكد من وجود local_uuid (مطلوب في بعض collections)
    if (!payload.containsKey('local_uuid') && !payload.containsKey('localUuid')) {
      payload['local_uuid'] = change.localUuid;
    }

    // ⭐ تحويل البيانات (يتولى إضافة الحقول المطلوبة و id للجداول الخاصة)
  ...
  /// ⭐ إصلاح جذري: عدم تحويل snake_case لأن Appwrite فعلياً يستخدم snake_case
  /// الحقول تأتي من delta_sync_service محولة إلى snake_case بالفعل
  /// و Appwrite schema يتطلب snake_case
  Map<String, dynamic> _sanitizePayload(Map<String, dynamic> payload,
    ...
    // ⭐ الحقول المطلوبة إضافياً (Appwrite required fields)
    ...
    for (final entry in payload.entries) {
      ...
      if (value is Map<String, dynamic>) {
        sanitized[key] = _sanitizePayload(value, collectionEntity: collectionEntity);
      } else {
        sanitized[key] = value;
      }
    }
    ...
    // ⭐ إضافة local_uuid إذا لم يكن موجوداً (مطلوب في بعض collections)
    if (!sanitized.containsKey('local_uuid') && payload.containsKey('localUuid')) {
