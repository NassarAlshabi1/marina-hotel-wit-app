...
class AppwriteDeltaSync {
  ...
  Future<bool> _shouldApplyRemote(
    ...
    // ✅ ضمان push-only بعد المغادرة: إذا تطابق lastModified المحلي مع البعيد
    // تماماً، فهذا يعني أن البيانات البعيدة هي نفسها التي رفعها هذا الجهاز
    // للتو. لا حاجة لإعادة كتابتها (توفير SQL + تجنّب أي تعارض محتمل).
    // هذا يمنع سيناريو "سحب بعد رفع" من إحداث كتابة مكررة على نفس السجل.
    if (localLastModified == remoteUpdatedAtSec) {
      return false;
    }
