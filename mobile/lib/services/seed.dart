...
class Seeder {
  ...
  Future<void> seedIfEmpty() async {
    ...
    for (final r in roomsCompanions) {
      ...
      await db.into(db.rooms).insert(r.copyWith(
          createdAt: d.Value(t),
          updatedAt: d.Value(t),
    ...
    final b1 = await db.into(db.bookings).insert(BookingsCompanion(
          ...
          checkinDate:
              d.Value(now.subtract(const Duration(days: 1)).toIso8601String()),
    ...
    await db.into(db.expenses).insert(ExpensesCompanion(
          ...
          amount: d.Value(21500),
          date:
              d.Value(Time.dateToString(now.subtract(const Duration(days: 1)))),
          localUuid: d.Value('x-2'),
          createdAt: d.Value(Time.nowEpoch()),
          updatedAt: d.Value(Time.nowEpoch()),
          lastModified: d.Value(Time.nowEpoch()),
          version: const d.Value(1),
    ...
    await db.into(db.payments).insert(PaymentsCompanion(
          ...
          paymentMethod: d.Value('نقدي'),
          revenueType: d.Value('room'),
          localUuid: d.Value('p-1'),
          createdAt: d.Value(Time.nowEpoch()),
          updatedAt: d.Value(Time.nowEpoch()),
          lastModified: d.Value(Time.nowEpoch()),
          version: const d.Value(1),
    ...
    await db.into(db.debts).insert(DebtsCompanion(
          ...
          guestName: d.Value('محمد علي'),
          checkinDate:
              d.Value(Time.dateToString(now.subtract(const Duration(days: 3)))),
          checkoutDate:
              d.Value(Time.dateToString(now.add(const Duration(days: 1)))),
