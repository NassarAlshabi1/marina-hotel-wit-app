...
class OutboxDao extends DatabaseAccessor<AppDatabase> with _$OutboxDaoMixin {
  ...
  /// يأخذ دفعة من السجلات المعلقة للمعالجة.
  ///
  /// يستخدم نمط "claim-first" الذري لتجنب race condition:
  /// 1. تحديث السجلات المعلقة لتصبح 'processing' في عملية واحدة (SQL خام مع LIMIT)
  /// 2. جلب السجلات التي تم حجزها بنجاح من خلال workerId
  ///
  /// هذا يضمن أن كل سجل يعالج مرة واحدة فقط حتى مع عاملين متزامنين.
  Future<List<OutboxData>> takeBatch(int limit, {String? workerId}) async {
    final worker = workerId ?? _uuid.v4();
    final nowEpoch = DateTime.now().millisecondsSinceEpoch ~/ 1000;

    return transaction(() async {
      // الخطوة 1: حجز السجلات ذرياً باستخدام SQL خام مع LIMIT
      // هذا يضمن أن UPDATE يتم ذرياً مع LIMIT
      const updateSql = '''
        UPDATE outbox 
      ...
      await customStatement(
        updateSql,
        ['processing', nowEpoch, worker, limit],
      );

      // الخطوة 2: جلب السجلات التي حجزناها نحن (بواسطة worker)
      final claimed = await (select(outbox)
            ..where((t) =>
                t.processingStatus.equals('processing') &
                t.processingWorker.equals(worker) &
                t.processingStartedAt.equals(nowEpoch))
  ...
  // ✅ الدالة الجديدة - حذف سجل واحد بالـ UUID
  ...
  Future<int> cleanupStuckEntries({
    ...
    // تحديث ذري واحد - تجنب race condition
