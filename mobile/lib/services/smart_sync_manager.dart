...
class SmartSyncManager {
  ...
  Future<void> initialize(GoogleDriveBackupService backupService) async {
    ...
    _log(
        '🔄 مدير المزامنة الذكي: تم تهيئة الغلاف بنجاح (Logic moved to Coordinator)');
  }
  ...
  Future<void> _loadSettings() async {
    ...
    final interval =
        prefs.getInt(_prefsIntervalKey) ?? _defaultSyncIntervalMinutes;
  ...
  Future<void> _notifySuccessfulSync(DriveBackupFile backup) async {
    // Kept private if needed or expose if Coordinator calls it
  ...
  Future<Map<String, dynamic>> getStatus() async {
    ...
    final coordStatus =
        await GoogleDriveUnifiedSyncCoordinator.instance.getStatus();
