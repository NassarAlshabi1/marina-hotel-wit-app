...
class ProgressCallback {
  ProgressCallback({this.onProgress, this.onComplete});
  final void Function(int current, int total, String status)? onProgress;
  final void Function(String message)? onComplete;
}

class GoogleAuthClient extends http.BaseClient {
  GoogleAuthClient(this._headers, {Duration? timeout}) 
      : _client = http.Client(),
...
class GoogleDriveBackupService {
  ...
  static const Duration downloadTimeout = Duration(minutes: 8);
  
  /// ⭐ إعدادات Batch Operations
  static const int defaultBatchSize = 20;
  static const int maxConcurrentBatches = 3;
  
  /// ⭐ إعدادات التنظيف التلقائي
  static const int maxDeltaFilesToKeep = 15;
  static const int maxAutoSyncFilesToKeep = 10;
  ...
  /// ⭐ تنفيذ عملية مع timeout وإعادة محاولة
  Future<T> _runWithAuth<T>(
    ...
    while (attempts < maxRetries) {
      try {
        ...
        attempts++;
        lastError = e;
        _log('⏱️ انتهت مهلة العملية (محاولة $attempts/$maxRetries): $e');
    ...
    throw TimeoutException(
      'فشلت العملية بعد $maxRetries محاولات: ${lastError.toString()}',
  ...
  /// ⭐ رفع نسخة احتياطية مع شريط تقدم
  Future<String> uploadBackup(
    ...
    return _runWithAuth<String>(() async {
      try {
        ...
        // ⭐ إرسال تقدم البدء
        _emitProgress(0, totalBytes, 'بدء رفع $typeLabel');
        progress?.onProgress?.call(0, totalBytes, 'بدء الرفع');

        _log(
          '📤 بدء رفع $typeLabel: $fileName (${(totalBytes / 1024).toStringAsFixed(2)} KB)',
        );

        // ⭐ استخدام Chunked Upload للملفات الكبيرة
        final uploadedFile = await _uploadWithProgress(
          driveFile,
          jsonBytes,
          progress,
        );
        ...
        // ⭐ إرسال تقدم التحقق
        _emitProgress(totalBytes, totalBytes, 'التحقق من الرفع');
        progress?.onProgress?.call(totalBytes, totalBytes, 'التحقق من الرفع');
        ...
        // ⭐ إرسال حدث الإكمال
        _emitSyncEvent(SyncEventType.uploadComplete, fileName, totalBytes);
        progress?.onComplete?.call('تم الرفع بنجاح');
        ...
        _emitSyncEvent(SyncEventType.error, e.toString(), 0);
  ...
  /// ⭐ رفع مع تتبع التقدم
  Future<drive.File> _uploadWithProgress(
    ...
    // للملفات الصغيرة، رفع مباشر
    if (bytes.length <= chunkSize) {
      final media = drive.Media(Stream.value(bytes), bytes.length);
      final result = await _driveApi!.files.create(driveFile, uploadMedia: media);
      _emitProgress(totalBytes, totalBytes, 'اكتمال الرفع');
      return result;
    }
    
    // للملفات الكبيرة، رفع بأجزاء مع تتبع التقدم
    ...
    // رفع الأجزاء (محاكاة - Google Drive API يدعم resumable upload)
    for (int i = 0; i < chunks.length; i++) {
      uploadedBytes += chunks[i].length;
      final progressPercent = (uploadedBytes / totalBytes * 100).toInt();
      _emitProgress(uploadedBytes, totalBytes, 'رفع الجزء ${i + 1}/${chunks.length} ($progressPercent%)');
      progress?.onProgress?.call(uploadedBytes, totalBytes, 'رفع الجزء ${i + 1}/${chunks.length}');
    }
  ...
  /// ⭐ إرسال تقدم
  void _emitProgress(int current, int total, String status) {
  ...
  /// ⭐ إرسال حدث مزامنة
  void _emitSyncEvent(SyncEventType type, String message, int bytes) {
  ...
  Future<String> uploadBackupWithName(
    ...
    return _runWithAuth<String>(() async {
      ...
      _emitProgress(0, bytes.length, 'بدء رفع $fileName');
      _log(
        '📤 رفع ملف مزامنة: $fileName (${(bytes.length / 1024).toStringAsFixed(2)} KB)',
      );
      
      final uploadedFile = await _uploadWithProgress(driveFile, bytes, progress);
      
      _emitProgress(bytes.length, bytes.length, 'اكتمال رفع $fileName');
  ...
  /// ⭐ تنظيف ملفات الدلتا القديمة تلقائياً
  Future<int> cleanupOldDeltaFiles({int keepCount = 15}) async {
    try {
      final backups = await listBackups();
      
      // فلترة ملفات الدلتا فقط
      final deltaFiles = backups
          .where((b) => b.fileName.startsWith('marina_sync_delta_'))
          .toList();
      
      if (deltaFiles.length <= keepCount) {
        _log('✅ عدد ملفات الدلتا (${deltaFiles.length}) أقل من الحد الأقصى ($keepCount)');
        return 0;
      }
      ...
      // حذف الملفات الزائدة (الأقدم)
      final toDelete = deltaFiles.take(deltaFiles.length - keepCount).toList();
      int deletedCount = 0;
      
      for (final file in toDelete) {
        try {
          await deleteBackupFile(file.fileId);
          deletedCount++;
          _log('🗑️ تم حذف ملف دلتا قديم: ${file.fileName}');
        } catch (e) {
          _log('⚠️ فشل حذف ${file.fileName}: $e');
        }
      }
      
      _log('✅ تم حذف $deletedCount ملف دلتا قديم');
      return deletedCount;
    } catch (e) {
      _log('❌ خطأ في تنظيف ملفات الدلتا: $e');
      return 0;
    }
  }

  /// ⭐ تنظيف ملفات المزامنة التلقائية القديمة
  Future<int> cleanupOldAutoSyncFiles({int keepCount = 10}) async {
    try {
      final backups = await listBackups();
      
      // فلترة ملفات المزامنة التلقائية فقط
      final autoSyncFiles = backups
          .where((b) => b.fileName.startsWith('marina_sync_auto_'))
          .toList();
      
      if (autoSyncFiles.length <= keepCount) {
        _log('✅ عدد ملفات المزامنة (${autoSyncFiles.length}) أقل من الحد الأقصى ($keepCount)');
        return 0;
      }
      
      // ترتيب من الأقدم للأحدث
      autoSyncFiles.sort((a, b) => a.createdTime.compareTo(b.createdTime));
      
      // حذف الملفات الزائدة (الأقدم)
      final toDelete = autoSyncFiles.take(autoSyncFiles.length - keepCount).toList();
      int deletedCount = 0;
      ...
      for (final file in toDelete) {
        try {
          await deleteBackupFile(file.fileId);
          deletedCount++;
          _log('🗑️ تم حذف ملف مزامنة قديم: ${file.fileName}');
        } catch (e) {
          _log('⚠️ فشل حذف ${file.fileName}: $e');
        }
      }
      
      _log('✅ تم حذف $deletedCount ملف مزامنة قديم');
      return deletedCount;
    } catch (e) {
      _log('❌ خطأ في تنظيف ملفات المزامنة: $e');
      return 0;
    }
  }

  /// ⭐ تنظيف شامل لجميع الملفات القديمة
  Future<Map<String, int>> performFullCleanup({
    int maxDeltaFiles = 15,
    int maxAutoSyncFiles = 10,
    int maxFullBackups = 5,
  }) async {
    final results = <String, int>{};
    
    _log('🧹 بدء التنظيف الشامل للملفات القديمة...');
    
    // تنظيف ملفات الدلتا
    results['delta_deleted'] = await cleanupOldDeltaFiles(keepCount: maxDeltaFiles);
    
    // تنظيف ملفات المزامنة التلقائية
    results['auto_sync_deleted'] = await cleanupOldAutoSyncFiles(keepCount: maxAutoSyncFiles);
    
    // تنظيف النسخ الكاملة القديمة
    results['full_backup_deleted'] = await cleanupOldBackups(
      maxBackupsToKeep: maxFullBackups,
    );
    
    final totalDeleted = results.values.fold(0, (sum, count) => sum + count);
    _log('✅ اكتمل التنظيف الشامل: تم حذف $totalDeleted ملف');
    
    return results;
  }

  /// ⭐ Batch Operations لرفع عدة ملفات بسرعة
  Future<List<String>> uploadBatchFiles(
    List<({String name, List<int> bytes, Map<String, String>? properties})> files, {
    int concurrency = 3,
    ProgressCallback? progress,
  }) async {
    ...
    _log('📤 بدء رفع ${files.length} ملف بـ concurrency=$concurrency');
      ...
      final futures = batch.map((file) async {
        try {
          final fileId = await uploadBackupWithName(
            file.name,
            file.bytes,
            appProperties: file.properties,
          );
          completed++;
          progress?.onProgress?.call(completed, files.length, 'تم رفع ${file.name}');
          return fileId;
        } catch (e) {
    ...
    _log('✅ تم رفع ${results.length}/${files.length} ملف بنجاح');
    if (errors.isNotEmpty) {
      _log('⚠️ فشل رفع ${errors.length} ملف: ${errors.join(', ')}');
    }
    
    progress?.onComplete?.call('اكتمل رفع ${results.length} ملف');
    return results;
  }

  /// ⭐ Batch Operations لتنزيل عدة ملفات بسرعة
  Future<List<Map<String, dynamic>>> downloadBatchFiles(
    List<String> fileIds, {
    int concurrency = 3,
    ProgressCallback? progress,
  }) async {
    ...
    _log('📥 بدء تنزيل ${fileIds.length} ملف بـ concurrency=$concurrency');
    
    for (int i = 0; i < fileIds.length; i += concurrency) {
      final batch = fileIds.skip(i).take(concurrency).toList();
      
      final futures = batch.map((fileId) async {
        try {
          final data = await downloadBackup(fileId);
          completed++;
          progress?.onProgress?.call(completed, fileIds.length, 'تم تنزيل $fileId');
          return data;
        } catch (e) {
          _log('⚠️ فشل تنزيل $fileId: $e');
          return null;
        }
      });
    ...
    _log('✅ تم تنزيل ${results.length}/${fileIds.length} ملف بنجاح');
    progress?.onComplete?.call('اكتمل تنزيل ${results.length} ملف');
    return results;
  }
...
class ProgressInfo {
  ...
  @override
  String toString() => 'Progress: $percent% ($current/$total) - $status';
}
