...
class CloudflareAiResult {
  const CloudflareAiResult({required this.answer, this.plan, this.rows = const [], this.requiresConfirmation = false});
  final String answer;
...
class CloudflareAiService {
  ...
  Future<CloudflareAiResult> _send(Map<String, dynamic> body) async {
    ...
    if (decoded is! Map<String, dynamic>) throw Exception('استجابة غير صالحة من Cloudflare');
    if (response.statusCode >= 400) throw Exception(decoded['error']?.toString() ?? 'فشل الطلب');
    return CloudflareAiResult(
      answer: decoded['answer']?.toString() ?? 'تم استلام الطلب.',
      plan: decoded['plan'] is Map ? Map<String, dynamic>.from(decoded['plan'] as Map) : null,
      requiresConfirmation: decoded['requires_confirmation'] == true,
      rows: decoded['rows'] is List
          ? (decoded['rows'] as List).whereType<Map>().map((row) => Map<String, dynamic>.from(row)).toList()
          : const [],
    );
  }
}
