...
//  Drop-in replacement for CloudflareSyncManager
//  Same interface: currentDeviceIdStatic, initialize(), sync(), etc.
// ═══════════════════════════════════════════════════════════════
...
// ─── Realtime sync state (replaces AppwriteRealtimeSync) ───────
...
class CloudflareSyncManager {
  ...
  // ─── Static device ID (same interface as CloudflareSyncManager) ──
  ...
  Future<void> initialize({AppDatabase? db, bool forceRetry = false}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _initError = null;
        dlog('✅ CloudflareSyncManager initialized — device: $_deviceId');
      } else {
        _initError = 'Login failed: ${response.statusCode}';
        dwarn(() => '⚠️ Cloudflare login failed: ${response.body}');
      }
    } catch (e) {
      _initError = 'Init error: $e';
      dwarn(() => '⚠️ CloudflareSyncManager init error: $e');
    }
  }
  ...
  Future<String> registerDevice() async {
    ...
    if (response.statusCode == 200) {
      dlog('✅ Device registered: $_deviceId');
      return _deviceId!;
    }
  ...
  Future<void> setFcmToken(String token) async {
    ...
    try {
      ...
      dlog('✅ FCM token set for device: $_deviceId');
    } catch (e) {
      dwarn(() => '⚠️ Set FCM token error: $e');
    }
  }
  ...
  Future<int> _pushOutbox() async {
    ...
    dlog('📤 Pushed $successCount/${pending.length} operations');
    return successCount;
  }
  ...
  Future<int> _pullChanges() async {
    ...
    dlog('📥 Pulled $totalPulled changes');
    return totalPulled;
  }
  ...
  Future<void> _applyChange(String entity, Map<String, dynamic> record) async {
    ...
    // Remove fields that shouldn't be written
    final cleanRecord = Map<String, dynamic>.from(record);
    cleanRecord.remove('id'); // Let Drift auto-generate
  ...
  void startAutoSync({Duration interval = const Duration(minutes: 15)}) {
    ...
    _autoSyncTimer = Timer.periodic(interval, (_) {
      sync().catchError((e) {
        dwarn(() => '⚠️ Auto-sync error: $e');
      });
    });
    dlog('⏰ Auto-sync started: every ${interval.inMinutes} minutes');
  }
  ...
  // ─── Audit log (stub — same interface as CloudflareSyncManager) ─
