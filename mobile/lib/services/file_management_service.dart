...
class FileManagementService {
  ...
  Future<Map<String, List<String>>> organizeBackupsByDate(List<String> backupPaths) async {
    ...
    for (final path in backupPaths) {
      try {
        ...
        final dateKey = '${stat.modified.year}-${stat.modified.month.toString().padLeft(2, '0')}-${stat.modified.day.toString().padLeft(2, '0')}';
  ...
  Future<void> _exportTableToCSV(Directory csvFolder, String tableName, String tableKey) async {
    try {
      ...
      final headers = firstRow.keys.map((key) => _translateColumnName(key)).join(',');
  ...
  String _formatRecordForDisplay(Map<String, dynamic> record, String tableType) {
    switch (tableType) {
      case 'rooms':
        return 'غرفة ${record['roomNumber'] ?? 'N/A'} - ${record['type'] ?? 'N/A'} - ${record['price'] ?? 'N/A'} ريال';
      case 'bookings':
        return '${record['guestName'] ?? 'N/A'} - غرفة ${record['roomNumber'] ?? 'N/A'} - ${record['checkinDate'] ?? 'N/A'}';
      case 'employees':
        return '${record['name'] ?? 'N/A'} - ${record['position'] ?? 'N/A'} - ${record['basicSalary'] ?? 'N/A'} ريال';
      case 'expenses':
        return '${record['description'] ?? 'N/A'} - ${record['amount'] ?? 'N/A'} ريال - ${record['date'] ?? 'N/A'}';
      case 'payments':
        return 'دفعة ${record['amount'] ?? 'N/A'} ريال - ${record['paymentMethod'] ?? 'N/A'} - ${record['paymentDate'] ?? 'N/A'}';
      case 'cash_transactions':
        return '${record['transactionType'] ?? 'N/A'} - ${record['amount'] ?? 'N/A'} ريال - ${record['description'] ?? 'N/A'}';
      default:
