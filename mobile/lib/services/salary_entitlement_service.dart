...
class SalaryEntitlementService {
  ...
  Future<SalaryEntitlement> calculateEmployeeEntitlement(
    ...
    for (final expense in expenses) {
      ...
      if (!ExpenseTypes.isSalaryType(type)) {
        continue;
      }
      // تخطي السلف المقسطة (تُنشأ تلقائياً بواسطة النظام)
      if (type == ExpenseTypes.salaryAdvance && expense.isAutoGenerated) {
        continue;
      }
      final normalized = ExpenseTypes.normalizeSalaryType(type);
      final isDeduction = normalized == ExpenseTypes.salaryDeduction;
