...
/// ⚠️ هذا يمنع خطأ "Unknown attribute" نهائياً
  ...
  Map<String, dynamic> _debtToRemote(Debt debt) {
    final data = <String, dynamic>{
      ...
      'guestName': (debt.guestName.isEmpty) ? 'غير محدد' : debt.guestName.trim(),
      'checkinDate': (debt.checkinDate?.isEmpty ?? true) ? null : debt.checkinDate!.trim(),
      'totalAmount': debt.totalAmount,
      'paidAmount': debt.paidAmount,
      'remainingAmount': debt.remainingAmount, // الآن double وليس integer
      'bookingLocalId': debt.bookingLocalId,
      'checkoutDate': (debt.checkoutDate?.isEmpty ?? true) ? null : debt.checkoutDate!.trim(),
      'paymentDate': (debt.paymentDate?.isEmpty ?? true) ? null : debt.paymentDate!.trim(),
      'isSettled': debt.isSettled,
      'debtReason': (debt.debtReason?.isEmpty ?? true) ? null : debt.debtReason!.trim(),
      'note': (debt.note?.isEmpty ?? true) ? null : debt.note!.trim(),
      'debtUuid': (debt.debtUuid?.isEmpty ?? true) ? null : debt.debtUuid!.trim(),
      'pledge': (debt.pledge?.isEmpty ?? true) ? null : debt.pledge!.trim(),
      'pledgeType': (debt.pledgeType?.isEmpty ?? true) ? null : debt.pledgeType!.trim(),
