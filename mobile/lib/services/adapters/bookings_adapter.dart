...
class BookingsAdapter extends EntityAdapter<Booking, BookingsCompanion> {
  ...
  Future<ResolveResult> resolveRefs(AppDatabase db, Map<String, dynamic> json, {required Source src}) async {
    final bookingUuid = _asString(json, 'localUuid', src) ?? _asString(json, 'booking_uuid', src) ?? _asString(json, 'bookingUuid', src);
  ...
  BookingsCompanion fromJson(Map<String, dynamic> json, {required Source src, required ResolveResult refs}) {
    ...
    return BookingsCompanion(
      ...
      guestName: _vStr(json, 'guestName', src) ?? _vStr(json, 'guest_name', src) ?? const d.Value.absent(),
      guestPhone: _vStr(json, 'guestPhone', src) ?? _vStr(json, 'guest_phone', src) ?? const d.Value.absent(),
      guestIdType: _vStr(json, 'guestIdType', src) ?? _vStr(json, 'guest_id_type', src) ?? const d.Value.absent(),
      guestIdNumber: _vStr(json, 'guestIdNumber', src) ?? _vStr(json, 'guest_id_number', src) ?? const d.Value.absent(),
      guestIdIssueDate: _vStr(json, 'guestIdIssueDate', src) ?? _vStr(json, 'guest_id_issue_date', src),
      guestIdIssuePlace: _vStr(json, 'guestIdIssuePlace', src) ?? _vStr(json, 'guest_id_issue_place', src),
      guestNationality: _vStr(json, 'guestNationality', src) ?? _vStr(json, 'guest_nationality', src) ?? const d.Value.absent(),
      guestEmail: _vStr(json, 'guestEmail', src) ?? _vStr(json, 'guest_email', src),
      guestAddress: _vStr(json, 'guestAddress', src) ?? _vStr(json, 'guest_address', src),
      checkinDate: _vStr(json, 'checkinDate', src) ?? _vStr(json, 'checkin_date', src) ?? const d.Value.absent(),
      checkoutDate: _vStr(json, 'checkoutDate', src) ?? _vStr(json, 'checkout_date', src),
      actualCheckout: _vStr(json, 'actualCheckout', src) ?? _vStr(json, 'actual_checkout', src),
      status: _vStr(json, 'status', src) ?? const d.Value.absent(),
      notes: _vStr(json, 'notes', src),
      expectedNights: _vInt(json, 'expectedNights', src) ?? _vInt(json, 'expected_nights', src) ?? const d.Value.absent(),
      calculatedNights: _vInt(json, 'calculatedNights', src) ?? _vInt(json, 'calculated_nights', src),
      totalNightsCached: _vInt(json, 'totalNightsCached', src),
      stayDurationIso: _vStr(json, 'stayDurationIso', src) ?? _vStr(json, 'stay_duration_iso', src),
      ...
      hotelDayCheckin: _vStr(json, 'hotelDayCheckin', src) ?? _vStr(json, 'hotel_day_checkin', src),
      hotelDayCheckout: _vStr(json, 'hotelDayCheckout', src) ?? _vStr(json, 'hotel_day_checkout', src),
  ...
  Map<String, dynamic> toJson(Booking model, {required Source src}) {
    return {
      ...
      _k(src, 'guestIdIssueDate', 'guest_id_issue_date'): model.guestIdIssueDate,
      _k(src, 'guestIdIssuePlace', 'guest_id_issue_place'): model.guestIdIssuePlace,
      ...
      _k(src, 'totalNightsCached', 'total_nights_cached'): model.totalNightsCached,
      ...
      _k(src, 'needsCheckoutReview', 'needs_checkout_review'): model.needsCheckoutReview,
      ...
      _k(src, 'remainingBalanceCached', 'remaining_balance_cached'): model.remainingBalanceCached,
