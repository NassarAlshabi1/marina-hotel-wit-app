...
class BookingsAdapter extends EntityAdapter<Booking, BookingsCompanion> {
  ...
  BookingsCompanion fromJson(Map<String, dynamic> json, {required Source src, required ResolveResult refs}) {
    ...
    return BookingsCompanion(
      ...
      guestPhone: _vStr(json, 'guestPhone', src, altKey: 'guest_phone', fallback: ''),
      guestIdType: _vStr(json, 'guestIdType', src, altKey: 'guest_id_type', fallback: 'بطاقة شخصية'),
      guestIdNumber: _vStr(json, 'guestIdNumber', src, altKey: 'guest_id_number', fallback: ''),
      guestIdIssueDate: _vStr(json, 'guestIdIssueDate', src, altKey: 'guest_id_issue_date'),
      guestIdIssuePlace: _vStr(json, 'guestIdIssuePlace', src, altKey: 'guest_id_issue_place'),
      guestNationality: _vStr(json, 'guestNationality', src, altKey: 'guest_nationality', fallback: ''),
      ...
      actualCheckout: d.Value(_asString(json, 'actualCheckout', src) ?? _asString(json, 'actual_checkout', src)),
      status: _vStr(json, 'status', src, fallback: ''),
      notes: _vStr(json, 'notes', src),
      discount: _vDouble(json, 'discount', src, altKey: 'discount'),
      discountType: _vStr(json, 'discountType', src, altKey: 'discount_type', fallback: 'per_night'),
      discountStartDate: _vStr(json, 'discountStartDate', src, altKey: 'discount_start_date'),
      expectedNights: _vInt(json, 'expectedNights', src, altKey: 'expected_nights', fallback: 1),
      calculatedNights: _vInt(json, 'calculatedNights', src, altKey: 'calculated_nights'),
      totalNightsCached: _vInt(json, 'totalNightsCached', src),
      stayDurationIso: _vStr(json, 'stayDurationIso', src, altKey: 'stay_duration_iso'),
      ...
      hotelDayCheckin: _vStr(json, 'hotelDayCheckin', src, altKey: 'hotel_day_checkin'),
      hotelDayCheckout: _vStr(json, 'hotelDayCheckout', src, altKey: 'hotel_day_checkout'),
...
String _k(Source src, String camel, [String? snake]) => src == Source.drive ? (snake ?? camel) : camel;
