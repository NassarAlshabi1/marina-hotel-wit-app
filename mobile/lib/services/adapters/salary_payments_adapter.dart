...
class SalaryPaymentsAdapter extends EntityAdapter<SalaryPayment, SalaryPaymentsCompanion> {
  ...
  SalaryPaymentsCompanion fromJson(Map<String, dynamic> json, {required Source src, required ResolveResult refs}) {
    ...
    return SalaryPaymentsCompanion(
      ...
      cycleId: _vInt(json, 'cycleId', src) ?? _vInt(json, 'cycle_id', src) ?? const d.Value.absent(),
      amount: _vDouble(json, 'amount', src) ?? const d.Value(0.0),
      hotelDayKey: _vStr(json, 'hotelDayKey', src) ?? _vStr(json, 'hotel_day_key', src),
      paymentDateIso: _vStr(json, 'paymentDateIso', src) ?? _vStr(json, 'payment_date_iso', src) ?? const d.Value(''),
