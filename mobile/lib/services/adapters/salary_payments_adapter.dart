...
class SalaryPaymentsAdapter
  ...
  @override
  SalaryPaymentsCompanion fromJson(Map<String, dynamic> json,
    ...
    return SalaryPaymentsCompanion(
      ...
      cycleId: _vInt(json, 'cycleId', src, fallback: 0) ??
          _vInt(json, 'cycle_id', src, fallback: 0) ??
          const d.Value(0),
      amount: _vDouble(json, 'amount', src, fallback: 0.0) ?? const d.Value(0.0),
      hotelDayKey:
          _vStr(json, 'hotelDayKey', src) ?? _vStr(json, 'hotel_day_key', src),
      paymentDateIso: _vStr(json, 'paymentDateIso', src, fallback: '') ??
          _vStr(json, 'payment_date_iso', src, fallback: '') ??
          const d.Value(''),
      method: _vStr(json, 'method', src),
      isAutoGenerated:
          _vBool(json, 'isAutoGenerated', src, fallback: false) ?? const d.Value(false),
