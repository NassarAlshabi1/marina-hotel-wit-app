...
class SalaryWithdrawalsAdapter
  ...
  SalaryWithdrawalsCompanion fromJson(
    ...
    final employeeId = _asInt(json, 'employeeId', src) ?? 
    ...
    final amount = amountInt?.toDouble() ?? _asDouble(json, 'amount', src) ?? 0.0;
  ...
  Map<String, dynamic> toJson(SalaryWithdrawal model, {required Source src}) {
    return {
      ...
      _k(src, 'amount', 'amount'): model.amount.round(), // ✅ integer for Appwrite
      ...
      _k(src, 'vectorClock', 'vector_clock'): 
          model.vectorClock?.isNotEmpty == true 
