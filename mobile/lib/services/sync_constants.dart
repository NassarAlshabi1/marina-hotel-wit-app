class SyncConstants {
  ...
  /// ⚠️ مُهمَل (2026-09-21) — Resumable Full Sync.
  ///
  /// كان الحد الأقصى لسجلات booking_nights عند السحب الأولي (1000)، لكنه
  /// تسبب في فقدان صامت للبيانات: المؤشر كان يُثبَّت من max($updatedAt)
  /// لأول 1000 سجل (الأقدم بـ $id) وتُعلن المجموعة مكتملة → الليالي الباقية
  /// لا تُسحب أبداً من أي Delta لاحق.
  ///
  /// **السلوك الجديد** (UnifiedPullEngine._runStreamingFullPull): السحب
  /// الأولي **بلا سقف** — تدفقي قابل للاستئناف (صفحة → تطبيق → cursor)،
  /// الذاكرة محدودة بصفحة واحدة، والانقطاع يُستأنف في الدورة التالية.
  ///
  /// الثابت محفوظ للتوافق الخلفي فقط — لا تستخدمه في كود جديد.
  @Deprecated('استخدم السحب التدفقي القابل للاستئناف — بلا سقف')
  static const int initialBookingNightsPullLimit = 1000;

  /// ✅ Resumable Full Sync: حجم دفعة الكتابة المجمّعة (db.batch) لليالي
  /// الحجوزات في مسار السحب — ذرّة واحدة لكل 250 سجل مع fallback صف-بصف.
  static const int bookingNightsApplyChunkSize = 250;
