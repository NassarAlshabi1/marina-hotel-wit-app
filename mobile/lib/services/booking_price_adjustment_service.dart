...
class BookingPriceAdjustmentService {
  ...
  Future<BookingPriceAdjustment> applyTemporaryAdjustment({
    ...
    final adjustment = BookingPriceAdjustmentsCompanion(
      ...
      amount: Value(amount),  // ⭐ int (already int)
  ...
  Future<LostRevenueReport> generateLostRevenueReport({
    ...
    if (fromHotelDay != null) {
      query
          .where((a) => a.effectiveHotelDay.isBiggerOrEqualValue(fromHotelDay));
    }
    if (toHotelDay != null) {
    ...
    final bookingIds =
        adjustments.map((a) => a.bookingLocalId).whereType<int>().toSet();
    ...
    for (final bookingId in bookingIds) {
      ...
      final bookingAdjustments =
          adjustments.where((a) => a.bookingLocalId == bookingId).toList();
      ...
      for (final adj in bookingAdjustments) {
        ...
        final double signedAmount = adj.amount.toDouble();  // ⭐ int to double
