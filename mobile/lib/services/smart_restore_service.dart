  ...
  Future<int> _mergeRooms(dynamic roomsData) async {
    ...
    for (final roomJson in roomsData) {
      try {
        ...
        final backupRoom = Room.fromJson(map, serializer: lenientValueSerializer);
        
        // البحث عن الغرفة باستخدام localUuid أو roomNumber
        final existingRoom = await (db.select(db.rooms)
          ..where((r) => r.localUuid.equals(backupRoom.localUuid) | r.roomNumber.equals(backupRoom.roomNumber)))
          .getSingleOrNull();
        
        if (existingRoom != null) {
          // تحديث الغرفة الموجودة
          await (db.update(db.rooms)..where((r) => r.id.equals(existingRoom.id)))
  ...
  Future<int> _mergeBookings(dynamic bookingsData) async {
    ...
    for (final bookingJson in bookingsData) {
      try {
        ...
        final backupBooking = Booking.fromJson(map, serializer: lenientValueSerializer);
        
        // التحقق من وجود الغرفة المرتبطة
        final roomExists = await _checkRoomExists(backupBooking.roomNumber);
        if (!roomExists) {
          _log('⚠️ تخطي حجز ${backupBooking.localUuid} - الغرفة ${backupBooking.roomNumber} غير موجودة');
          continue;
        }
        ...
        if (existingBooking != null) {
          ...
          // تحديث الحجز الموجود
          await (db.update(db.bookings)..where((b) => b.id.equals(existingBooking.id)))
            .write(BookingsCompanion(
              ...
              hotelDayKey: Value(backupBooking.hotelDayKey),
              updatedAt: Value(Time.nowEpoch()),
              lastModified: Value(Time.nowEpoch()),
            ));
        } else {
          ...
          localId = await db.into(db.bookings).insert(BookingsCompanion.insert(
            ...
            guestPhone: backupBooking.guestPhone,
            guestNationality: Value(backupBooking.guestNationality),
            guestIdType: Value(backupBooking.guestIdType),
            guestIdNumber: Value(backupBooking.guestIdNumber),
            checkinDate: backupBooking.checkinDate,
            checkoutDate: backupBooking.checkoutDate,
            actualCheckout: Value(backupBooking.actualCheckout),
    ...
    _log('📊 تم دمج $count حجز، خريطة UUID: ${_bookingUuidToLocalId.length} عنصر');
    return count;
  }
  ...
  Future<int> _mergeBookingNotes(dynamic notesData) async {
    ...
    for (final noteJson in notesData) {
      try {
        ...
        final backupNote = BookingNote.fromJson(map, serializer: lenientValueSerializer);
        ...
        if (map.containsKey('bookingUuidCache') && map['bookingUuidCache'] != null) {
        ...
        if (existingNote != null) {
          await (db.update(db.bookingNotes)..where((n) => n.id.equals(existingNote.id)))
            .write(BookingNotesCompanion(
              bookingId: Value(correctBookingId ?? existingNote.bookingId),
              note: Value(backupNote.note),
              createdBy: Value(backupNote.createdBy),
              updatedAt: Value(Time.nowEpoch()),
              lastModified: Value(Time.nowEpoch()),
            ));
        } else {
  ...
  Future<int> _mergeBookingNights(dynamic nightsData) async {
    ...
    for (final nightJson in nightsData) {
      try {
        ...
        final backupNight = BookingNight.fromJson(map, serializer: lenientValueSerializer);
        ...
        if (existingNight != null) {
          await (db.update(db.bookingNights)..where((n) => n.id.equals(existingNight.id)))
            .write(BookingNightsCompanion(
              bookingLocalId: Value(correctBookingId),
              hotelDayKey: Value(backupNight.hotelDayKey),
              nightDate: Value(backupNight.nightDate),
              roomNumber: Value(backupNight.roomNumber),
              nightPrice: Value(backupNight.nightPrice),
              nightIndex: Value(backupNight.nightIndex),
              updatedAt: Value(Time.nowEpoch()),
              lastModified: Value(Time.nowEpoch()),
            ));
        } else {
          await db.into(db.bookingNights).insert(BookingNightsCompanion.insert(
            ...
            nightDate: backupNight.nightDate,
            roomNumber: backupNight.roomNumber,
            nightPrice: backupNight.nightPrice,
            nightIndex: Value(backupNight.nightIndex),
  ...
  Future<int> _mergeLedger(dynamic ledgerData) async {
    ...
    for (final ledgerJson in ledgerData) {
      try {
        ...
        final backupLedger = HotelDayLedgerEntry.fromJson(map, serializer: lenientValueSerializer);
        
        // البحث باستخدام localUuid أو hotelDayKey
        final existingLedger = await (db.select(db.hotelDayLedger)
          ..where((l) => l.localUuid.equals(backupLedger.localUuid) | l.hotelDayKey.equals(backupLedger.hotelDayKey)))
          .getSingleOrNull();
        
        if (existingLedger != null) {
          await (db.update(db.hotelDayLedger)..where((l) => l.id.equals(existingLedger.id)))
            .write(HotelDayLedgerCompanion(
              dayDate: Value(backupLedger.dayDate),
              openedAt: Value(backupLedger.openedAt),
              closedAt: Value(backupLedger.closedAt),
              openingBalance: Value(backupLedger.openingBalance),
              closingBalance: Value(backupLedger.closingBalance),
              totalIncome: Value(backupLedger.totalIncome),
              totalExpenses: Value(backupLedger.totalExpenses),
              totalSalaries: Value(backupLedger.totalSalaries),
              roomRevenue: Value(backupLedger.roomRevenue),
              otherRevenue: Value(backupLedger.otherRevenue),
              netProfit: Value(backupLedger.netProfit),
              occupiedRooms: Value(backupLedger.occupiedRooms),
              availableRooms: Value(backupLedger.availableRooms),
              checkinsCount: Value(backupLedger.checkinsCount),
              checkoutsCount: Value(backupLedger.checkoutsCount),
              status: Value(backupLedger.status),
              notes: Value(backupLedger.notes),
              updatedAt: Value(Time.nowEpoch()),
              lastModified: Value(Time.nowEpoch()),
            ));
        } else {
          await db.into(db.hotelDayLedger).insert(HotelDayLedgerCompanion.insert(
            localUuid: backupLedger.localUuid,
            hotelDayKey: backupLedger.hotelDayKey,
            dayDate: backupLedger.dayDate,
            openedAt: Value(backupLedger.openedAt),
            closedAt: Value(backupLedger.closedAt),
            openingBalance: Value(backupLedger.openingBalance),
            closingBalance: Value(backupLedger.closingBalance),
  ...
  Future<int> _mergeEmployees(dynamic employeesData) async {
    ...
    for (final employeeJson in employeesData) {
      try {
        ...
        final backupEmployee = Employee.fromJson(map, serializer: lenientValueSerializer);
        ...
        if (existingEmployee != null) {
          await (db.update(db.employees)..where((e) => e.id.equals(existingEmployee.id)))
  ...
  Future<int> _mergeExpenses(dynamic expensesData) async {
    ...
    for (final expenseJson in expensesData) {
      try {
        ...
        final backupExpense = Expense.fromJson(map, serializer: lenientValueSerializer);
        ...
        if (existingExpense != null) {
          await (db.update(db.expenses)..where((e) => e.id.equals(existingExpense.id)))
            .write(ExpensesCompanion(
              ...
              expenseDate: Value(backupExpense.expenseDate),
              hotelDayKey: Value(backupExpense.hotelDayKey),
              addedBy: Value(backupExpense.addedBy),
              updatedAt: Value(Time.nowEpoch()),
              lastModified: Value(Time.nowEpoch()),
            ));
        } else {
  ...
  Future<int> _mergeCashTransactions(dynamic transactionsData) async {
    ...
    for (final transactionJson in transactionsData) {
      try {
        ...
        final backupTransaction = CashTransaction.fromJson(map, serializer: lenientValueSerializer);
        ...
        if (existingTransaction != null) {
          ...
          await (db.update(db.cashTransactions)..where((t) => t.id.equals(existingTransaction.id)))
          ...
          localId = await db.into(db.cashTransactions).insert(CashTransactionsCompanion.insert(
            localUuid: backupTransaction.localUuid,
            registerId: backupTransaction.registerId,
  ...
  Future<int> _mergePayments(dynamic paymentsData) async {
    ...
    for (final paymentJson in paymentsData) {
      try {
        ...
        final backupPayment = Payment.fromJson(map, serializer: lenientValueSerializer);
        ...
        if (backupPayment.bookingUuidCache != null && 
          ...
          correctBookingId = _bookingUuidToLocalId[backupPayment.bookingUuidCache];
        }
        
        // ثانياً: البحث عن الحجز باستخدام localUuid من البيانات المخزنة
        if (correctBookingId == null && map.containsKey('booking_uuid') && map['booking_uuid'] != null) {
        ...
        if (correctBookingId == null && backupPayment.bookingLocalId != null) {
          _log('⚠️ تخطي دفعة ${backupPayment.localUuid} - لم يُعثر على الحجز المرتبط');
          skipped++;
          continue;
        }
        ...
        if (map.containsKey('cash_transaction_uuid') && map['cash_transaction_uuid'] != null) {
        ...
        if (existingPayment != null) {
          ...
          await (db.update(db.payments)..where((p) => p.id.equals(existingPayment.id)))
            .write(PaymentsCompanion(
              serverPaymentId: Value(backupPayment.serverPaymentId),
              bookingLocalId: Value(correctBookingId ?? existingPayment.bookingLocalId),
              ...
              cashTransactionLocalId: Value(correctCashTransactionId ?? existingPayment.cashTransactionLocalId),
              cashTransactionServerId: Value(backupPayment.cashTransactionServerId),
  ...
  Future<int> _mergeDebts(dynamic debtsData) async {
    ...
    for (final debtJson in debtsData) {
      try {
        ...
        final backupDebt = Debt.fromJson(map, serializer: lenientValueSerializer);
        ...
        if (existingDebt != null) {
          await (db.update(db.debts)..where((d) => d.id.equals(existingDebt.id)))
            .write(DebtsCompanion(
              bookingLocalId: Value(correctBookingId ?? existingDebt.bookingLocalId),
      ...
      for (final booking in allBookings) {
        try {
          ...
          final totalPaidCents = payments.fold<int>(
            0, 
            (sum, payment) => sum + (payment.amount * 100).round()
          );
          ...
          if (booking.totalDueCached != totalDue ||
            ...
            _log('💰 تحديث حجز #${booking.id}: المستحق=$totalDue، المدفوع=$totalPaid، المتبقي=$remainingBalance');
          }
