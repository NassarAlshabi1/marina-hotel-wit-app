...
class GoogleDriveUnifiedSyncCoordinator {
  ...
  final RetryStrategy _retryStrategy =
      RetryStrategy(config: RetryConfig.balanced);
    ...
    return (_database!.select(_database!.outbox)).watch().listen(
      ...
      onError: (error) {
        ...
        if (_outboxRetryCount > _maxOutboxRetries) {
          _log(
              '🛑 Max outbox retries reached ($_maxOutboxRetries). Stopping auto-retry.',
      ...
      onDone: () {
        ...
        Future.delayed(delay, () {
          if (_pushEnabled &&
              _database != null &&
              _backupService?.isSignedIn == true) {
            _restartOutboxMonitoring();
          }
        });
      },
  ...
  Future<void> notifyLocalChange(
      {String? table, String? operation, int count = 1}) async {
  ...
  Future<SyncResult> _performSyncLocked(
      SyncTrigger trigger, SyncMode mode) async {
  ...
  Future<SyncResult> performSyncWithRetry({
    ...
    return _retryStrategy.execute<SyncResult>(
      ...
      shouldRetry: (error) {
        ...
        if (errorStr.contains('quotaexceeded') ||
  ...
  Future<void> _performScanAndRestoreFullBackup() async {
    ...
    try {
      ...
      if (lastRestoredTs != null &&
          latestBackup.createdTime.isBefore(lastRestoredTs)) {
      ...
      _log(
          '⚠️ Full restore requires stopping all operations and cannot be done during sync.');
      ...
      await prefs.setString('pending_restore_timestamp',
          latestBackup.createdTime.toIso8601String());
      await prefs.setBool('pending_restore_available', true);

      _log('✅ Backup flagged for deferred restore.');
      _log(
          'ℹ️ User will be prompted to restore at next app launch or manually.');
