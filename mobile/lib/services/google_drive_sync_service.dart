...
class GoogleDriveSyncService {
  GoogleDriveSyncService({
    ...
    int shardSizeBytes = _kDefaultShardBytes,
            ...
            googleSignIn ?? GoogleDriveSignInManager.instance.client,
        _driveApi = driveApi,
  ...
  Future<DriveSyncIndex> uploadSnapshot({
    ...
    final normalizedSnapshot =
        SyncSnapshot(metadata: normalizedMetadata, tables: snapshot.tables);
    ...
    final uploadedShards =
        await _uploadShards(api, shards, normalizedMetadata, deviceId);
  ...
  Future<List<DriveSyncShard>> _uploadShards(
    ...
    for (var index = 0; index < shards.length; index++) {
      ...
      final media =
          drive.Media(Stream.value(shards[index]), shards[index].length);
      final created = await api.files.create(file, uploadMedia: media);
  ...
  Future<void> _cleanupLegacySnapshot(drive.DriveApi api,
      {required List<String> keepShardIds}) async {
    final result = await api.files.list(
      spaces: 'appDataFolder',
      q: "name contains 'sync_data' and trashed=false",
      $fields: 'files(id)',
  ...
  Future<Uint8List> _decodePayload(Uint8List bytes) async {
    ...
    final aes = encrypt.Encrypter(
        encrypt.AES(encrypt.Key(key), mode: encrypt.AESMode.cbc));
    final decrypted =
        aes.decryptBytes(encrypt.Encrypted(cipher), iv: encrypt.IV(ivBytes));
