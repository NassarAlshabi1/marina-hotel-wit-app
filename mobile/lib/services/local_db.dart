...
class ShiftNotes extends Table {
  ...
  TextColumn get shiftType => text()
      .withDefault(const Constant('all'))(); // morning, evening, night, all
...
class SalaryPayments extends Table with SyncFields {
  ...
  List<Index> get indexes => [
        Index('idx_salary_payments_cycle',
...
class AppDatabase extends _$AppDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
        ...
        onUpgrade: (m, from, to) async {
          ...
          if (from < 3) {
            await m.database
                .customStatement('ALTER TABLE rooms RENAME TO rooms_old');
    ...
    await transaction(() async {
      Future<void> replaceTableInBatches<T extends Insertable<dynamic>>(
        ...
        for (var i = 0; i < rows.length; i += batchSize) {
          ...
          await batch((batch) {
            batch.insertAll(table, batchRows.map(fromJson).toList(),
                mode: InsertMode.insertOrReplace);
          });
      ...
      await replaceTableInBatches<Room>(
          rooms, 'rooms', (row) => Room.fromJson(row));
      await replaceTableInBatches<Booking>(
          bookings, 'bookings', (row) => Booking.fromJson(row));
      await replaceTableInBatches<BookingNote>(
          bookingNotes, 'booking_notes', (row) => BookingNote.fromJson(row));
      await replaceTableInBatches<Employee>(
          employees, 'employees', (row) => Employee.fromJson(row));
      await replaceTableInBatches<Expense>(
          expenses, 'expenses', (row) => Expense.fromJson(row));
      await replaceTableInBatches<CashTransaction>(cashTransactions,
          'cash_transactions', (row) => CashTransaction.fromJson(row));
      await replaceTableInBatches<Payment>(
          payments, 'payments', (row) => Payment.fromJson(row));
      await replaceTableInBatches<Debt>(
          debts, 'debts', (row) => Debt.fromJson(row));
      await replaceTableInBatches<BookingNight>(
          bookingNights, 'booking_nights', (row) => BookingNight.fromJson(row));
      await replaceTableInBatches<HotelDayLedgerEntry>(hotelDayLedger,
          'hotel_day_ledger', (row) => HotelDayLedgerEntry.fromJson(row));
      await replaceTableInBatches<AutoFixRun>(
          autoFixRuns, 'auto_fix_runs', (row) => AutoFixRun.fromJson(row));
      await replaceTableInBatches<IntegrityViolation>(integrityViolations,
          'integrity_violations', (row) => IntegrityViolation.fromJson(row));
      await replaceTableInBatches<AppSession>(
          appSessions, 'app_sessions', (row) => AppSession.fromJson(row));
      await replaceTableInBatches<SalaryCycle>(
          salaryCycles, 'salary_cycles', (row) => SalaryCycle.fromJson(row));
      await replaceTableInBatches<SalaryPayment>(salaryPayments,
          'salary_payments', (row) => SalaryPayment.fromJson(row));
    });
  }
}
...
class DatabaseManager {
  ...
  static AppDatabase get instance {
    if (_isRestoring) {
      throw StateError(
          'Database is being restored. Operations are temporarily suspended.');
    }
    if (_isClosed) {
      developer.log('⚠️ Attempted to access closed database. Auto-reopening...',
  ...
  static bool get isInitialized =>
      _instance != null && !_isClosed && !_isClosing && !_isRestoring;
  ...
  static void registerReopenCallback(Function() callback) {
    if (!_onReopenCallbacks.contains(callback)) {
      ...
      developer.log(
          '📝 Registered reopen callback (total: ${_onReopenCallbacks.length})',
          name: 'DatabaseManager');
    }
  }
  ...
  static void unregisterReopenCallback(Function() callback) {
    ...
    developer.log(
        '📝 Unregistered reopen callback (total: ${_onReopenCallbacks.length})',
        name: 'DatabaseManager');
  }
  ...
  static Future<void> close() async {
    return _lock.synchronized(() async {
      if (_isClosing || _isClosed) {
        developer.log('Database is already closing or closed',
            name: 'DatabaseManager');
        return;
      }
      ...
      try {
        if (_instance != null) {
          ...
          developer.log('✅ Database closed successfully',
              name: 'DatabaseManager');
        }
      } catch (e, stack) {
        developer.log('❌ Error closing database: $e',
            name: 'DatabaseManager', error: e, stackTrace: stack);
      } finally {
        _instance = null;
        _isClosed = true;
        _isClosing = false;
  ...
  static Future<void> reopen() async {
    developer.log('🔄 Attempting to reopen database...',
        name: 'DatabaseManager');

    if (_isClosing) {
      developer.log('⏳ Waiting for database to finish closing...',
          name: 'DatabaseManager');
    ...
    if (_instance != null && !_isClosed) {
      try {
        developer.log('🔒 Closing existing database connection...',
            name: 'DatabaseManager');
        await close();
      } catch (e) {
        developer.log('⚠️ Error closing existing connection: $e',
            name: 'DatabaseManager');
      }
    }
    ...
    try {
      ...
      developer.log('✅ Database reopened successfully',
          name: 'DatabaseManager');

      // استدعاء جميع callbacks المسجلة
      if (_onReopenCallbacks.isNotEmpty) {
        developer.log(
            '🔔 Notifying ${_onReopenCallbacks.length} reopen callbacks...',
            name: 'DatabaseManager');
        for (final callback in _onReopenCallbacks) {
          try {
            callback();
          } catch (e) {
            developer.log('⚠️ Reopen callback error: $e',
                name: 'DatabaseManager');
          }
        }
        developer.log('✅ All reopen callbacks notified',
            name: 'DatabaseManager');
      }
    } catch (e, stack) {
      developer.log('❌ Failed to reopen database: $e',
          name: 'DatabaseManager', error: e, stackTrace: stack);
  ...
  static Future<T> withDatabase<T>(
      Future<T> Function(AppDatabase db) operation) async {
    if (_isClosed || _isClosing || _isRestoring) {
      throw StateError(
          'Cannot perform database operation: database is closed, closing, or being restored');
    }
  ...
  static Future<void> closeForRestore() async {
    return _lock.synchronized(() async {
      ...
      if (_isClosing || _isClosed) {
        developer.log('Database is already closing/closed',
            name: 'DatabaseManager');
      }

      _isRestoring = true;
      developer.log('🔒 Starting safe database closure for restore...',
          name: 'DatabaseManager');
      ...
      developer.log('✅ Database safely closed for restore',
          name: 'DatabaseManager');
    });
  }
  ...
  static Future<void> reopenAfterRestore() async {
    return _lock.synchronized(() async {
      ...
      developer.log('🔄 Reopening database after restore...',
          name: 'DatabaseManager');

      try {
        await reopen();
        await _restartAllSyncOperations();
        developer.log('✅ Database reopened and sync restarted',
            name: 'DatabaseManager');
      } finally {
        _isRestoring = false;
      }
  ...
  static Future<void> _stopAllSyncOperations() async {
    developer.log('⏸️ Stopping all sync operations...',
        name: 'DatabaseManager');

    if (_stopSyncCallback != null) {
      try {
        await _stopSyncCallback!();
        developer.log('✅ Sync operations stopped via callback',
            name: 'DatabaseManager');
      } catch (e) {
        developer.log('⚠️ Error in stop sync callback: $e',
            name: 'DatabaseManager');
      }
    } else {
      developer.log('ℹ️ No stop sync callback registered',
          name: 'DatabaseManager');
    }
  ...
  static Future<void> _waitForPendingOperations() async {
    developer.log('⏳ Waiting for $_activeOperations pending operations...',
        name: 'DatabaseManager');
    if (_activeOperations > 0) {
      ...
      await _operationsCompleter!.future.timeout(
        ...
        onTimeout: () {
          developer.log('⚠️ Timed out waiting for pending operations.',
              name: 'DatabaseManager');
        },
      );
    }
    developer.log('✅ Pending operations wait completed',
        name: 'DatabaseManager');
  }
  ...
  static void _releaseOperation() {
    _activeOperations--;
    if (_activeOperations == 0 &&
        _operationsCompleter != null &&
        !_operationsCompleter!.isCompleted) {
  ...
  static Future<void> _restartAllSyncOperations() async {
    developer.log('▶️ Restarting all sync operations...',
        name: 'DatabaseManager');

    if (_restartSyncCallback != null) {
      try {
        await _restartSyncCallback!();
        developer.log('✅ Sync operations restarted via callback',
            name: 'DatabaseManager');
      } catch (e) {
        developer.log('⚠️ Error in restart sync callback: $e',
            name: 'DatabaseManager');
      }
    } else {
      developer.log('ℹ️ No restart sync callback registered',
          name: 'DatabaseManager');
    }
  }
}
    ...
    return _db.transaction(() async {
      final logId = await _db.into(_db.syncLog).insert(
            SyncLogCompanion.insert(
              ...
              operations: jsonEncode(appliedOperations
                  .map((sync_models.SyncOperation e) => {
                        'table': e.table,
                        'uuid': e.uuid,
