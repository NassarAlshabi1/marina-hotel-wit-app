  ...
  Future<void> applyMergedData(Map<String, dynamic> merged) async {
    await transaction(() async {
      ...
      await replaceTable<CashTransaction>(
        cashTransactions,
        _asList('cash_transactions').map((row) => CashTransaction.fromJson(row)).toList(),
      );
      ...
      await replaceTable<BookingNight>(
        bookingNights,
        _asList('booking_nights').map((row) => BookingNight.fromJson(row)).toList(),
      );
      await replaceTable<HotelDayLedgerEntry>(
        hotelDayLedger,
        _asList('hotel_day_ledger').map((row) => HotelDayLedgerEntry.fromJson(row)).toList(),
      );
      await replaceTable<AutoFixRun>(
        autoFixRuns,
        _asList('auto_fix_runs').map((row) => AutoFixRun.fromJson(row)).toList(),
      );
      await replaceTable<IntegrityViolation>(
        integrityViolations,
        _asList('integrity_violations').map((row) => IntegrityViolation.fromJson(row)).toList(),
      );
      await replaceTable<AppSession>(
        appSessions,
        _asList('app_sessions').map((row) => AppSession.fromJson(row)).toList(),
      );
      await replaceTable<SalaryCycle>(
        salaryCycles,
        _asList('salary_cycles').map((row) => SalaryCycle.fromJson(row)).toList(),
      );
      await replaceTable<SalaryPayment>(
        salaryPayments,
        _asList('salary_payments').map((row) => SalaryPayment.fromJson(row)).toList(),
      );
    });
  }
}
