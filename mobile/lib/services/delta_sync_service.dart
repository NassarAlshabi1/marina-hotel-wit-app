...
class DeltaSyncService {
  ...
  Future<void> _persistMirrorSnapshot(
    ...
    // ⭐ تحسين: استخدام transaction واحد لجميع العمليات
    await db.transaction(() async {
      for (final entry in snapshot.entries) {
        ...
        // ⭐ تحسين: تجميع السجلات وإدخالها في دفعة واحدة
        final values = entry.value.values.toList();
        final batchInsert = StringBuffer();
        
        for (int i = 0; i < values.length; i++) {
          if (i > 0) batchInsert.write(',');
          batchInsert.write("(?, ?, ?, ?, ?)");
        }
        ...
        if (values.isNotEmpty) {
          ...
          for (final row in values) {
            args.addAll([table, row.localUuid, row.rowHash, jsonEncode(row.payload), row.lastSeenAt]);
          }
          
          await db.customStatement(
            'INSERT OR REPLACE INTO sync_mirror (table_name, local_uuid, row_hash, payload, last_seen_at) VALUES ${batchInsert.toString()}',
            args,
          );
        }
      }
  ...
  /// ⭐ تحسين: التخزين المؤقت للـ configs
  List<_EntityConfig>? _cachedConfigs;
  
  List<_EntityConfig> _entityConfigs() {
    // إعادة استخدام الـ configs المحفوظة
    ...
    _cachedConfigs = [
      ...
      // ⭐ تم إزالة التكرار: booking_price_adjustments موجود بالفعل في الأعلى (السطر 456)
      _EntityConfig(
        entity: 'audit_logs',
...
/// نموذج لتمثيل تغيير في المزامنة (مُحسَّن)
class DeltaSyncChangeOptimized {
  DeltaSyncChangeOptimized({
    required this.entity,
    required this.operation,
    required this.data,
    required this.rowHash,
