...
class CloudflareConfig {
  ...
  /// المستخدم: تسريع السحب الكامل عند أول تثبيت): ~7,300 صف.
  /// ✅ (2026-09-15) 400 → 500 = السقف الخادمي MAX_PULL_BATCH_SIZE
  /// نفسه: ~7,300 صف ≈ 15 طلباً بدل 19 — كل طلب أقل يعني رحلة ذهاب
  /// وإياب أقل + JSON أقل على جهاز ضعيف.
  static const int fullPullBatchSize = 500;

  /// ✅ (2026-09-15) صفحة سحب الدلتا — مستقلة عن [batchSize] (الذي
  /// يبقى سقف دفع outbox عند 100). طلب المستخدم: تسريع الدلتا أيضاً —
  /// بعد يوم مزدحم قد تتراكم مئات التغييرات من الأجهزة الأخرى؛ 250
  /// يقلّص الطلبات للنصف تقريباً مع بقاء الطلب خفيفاً على الشبكات
  /// الضعيفة (اليمن). السقف الخادمي 500 يحمي من المبالغة.
  static const int deltaPullBatchSize = 250;
}
