const { Client, Databases } = require('node-appwrite');

const ENDPOINT = 'https://fra.cloud.appwrite.io/v1';
const PROJECT_ID = '690ff0da0025518570c1';
const API_KEY = 'standard_';
const DATABASE_ID = 'hotel_db';

const client = new Client()
    .setEndpoint(ENDPOINT)
    .setProject(PROJECT_ID)
    .setKey(API_KEY);

const databases = new Databases(client);

const newFields = ['created_at', 'updated_at', 'deleted_at', 'last_modified', 'version', 'origin', 'vector_clock'];

async function main() {
    console.log('🔍 Checking newly created fields status...\n');
    
    for (const coll of ['bookings', 'payments', 'debts']) {
        console.log(`📦 ${coll}:`);
        const attrs = await databases.listAttributes(DATABASE_ID, coll);
        
        for (const f of newFields) {
            const attr = attrs.attributes.find(a => a.key === f);
            if (attr) {
                console.log(`   ${attr.status === 'available' ? '✅' : '⏳'} ${f}: ${attr.status || 'available'}`);
            }
        }
        console.log('');
    }
}

main().catch(console.error);
